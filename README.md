...
**To understand how it is decided.** Read the *Guía rápida* section of [`docs/research/Plan de análisis congelado.md`](docs/research/Plan%20de%20an%C3%A1lisis%20congelado.md), the pre-registered analysis plan, frozen before any result existed. That section is the plain-language version and is meant to be read on its own. If you need the statistics themselves explained, the last block of [`docs/research/Conceptos.md`](docs/research/Conceptos.md) covers each tool the plan uses, one entry per tool, with a worked example. Use it as a dictionary, not front to back.
