...
El término $M^{(4)}/(M^{(2)})^2 - 3$ es el exceso de curtosis (curtosis menos 3, donde 3 corresponde a la normal). El parámetro $\beta = 1.2$ se elige para compensar el exceso de curtosis de una distribución uniforme sobre $[-1,1]$, que es $-1.2$ y representa el caso límite no esperado en la práctica; sumar $\beta = 1.2$ deja así el denominador no-negativo en el límite uniforme; estrictamente positivo gracias al clamp, evitando divisiones por cero. La corrección por curtosis se justifica por la teoría long-tail del deep learning: muestras raras o atípicas tienen influencia desproporcionada y producen distribuciones leptocúrticas que aumentan la curtosis, reduciendo GWA y señalando patrones de aprendizaje problemáticos.
