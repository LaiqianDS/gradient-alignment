...
**Coste.** Barato en el estimador canónico del paper, de tramo medio en la v1 del TFG. En el estimador del paper la extracción de $z_i$ y de los logits es parte del forward estándar; los labels $y_i$ están disponibles en el batch. No se requiere un backward adicional para esa variante porque $\mathbf{g}_t(x_i)$ se compone en forma cerrada como producto exterior de tensores ya materializados. El coste dominante por minibatch es $\mathcal{O}(B \cdot d_z \cdot C)$, donde $B$ es el batch size, $d_z$ la dimensión latente y $C$ el número de clases. La v1 del TFG no usa esa forma cerrada: obtiene los cosenos del barrido per-sample compartido, así que su coste real es el de ese barrido (ver §Uso en el TFG).
...
- **Paper comparable más reciente (delta a argumentar).** Junto con Ru et al. (2021, TSE) y Forouzesh & Thiran (2021, Gradient Disparity), es uno de los tres papers más comparables al TFG (mismo problema: proxy train-time barato de generalización) y el más reciente. La intro de la tesis debe argumentar el delta frente a este: GWA es un proxy puntual *post-hoc* para early stopping y selección de modelo sobre una métrica/arquitectura; el TFG propone un estudio correlacional cerrado que mide 8 métricas (5 alineación + 3 varianza) + baseline en ventanas tempranas (5/10/25/50% de épocas), sobre gradiente bruto $\nabla L$ (no solo last-layer), barriendo SGD/Adam y FC/CNN/ResNet, para predecir generalización y eficiencia sin optimizar nada.
...
- **Coste y ubicación en el sweep.** En la v1 del TFG GWA sí consume el barrido per-sample $\nabla L$ compartido: `metrics/gwa.py` expone `reduce(sweep)` y lee `sweep.gwa_cos`, y su ruta `compute` recorre `iter_per_sample_grad_dicts`. Su coste marginal no es despreciable, es el del barrido compartido. La forma cerrada vía hooks al forward, que dejaría la métrica casi gratis, queda como optimización pendiente.
