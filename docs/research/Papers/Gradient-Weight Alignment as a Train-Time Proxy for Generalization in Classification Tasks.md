...
El término $M^{(4)}/(M^{(2)})^2 - 3$ es el exceso de curtosis (curtosis menos 3, donde 3 corresponde a la normal). El parámetro $\beta = 1.2$ se elige para compensar el exceso de curtosis de una distribución uniforme sobre $[-1,1]$, que es $-1.2$ y representa el caso límite no esperado en la práctica; sumar $\beta = 1.2$ garantiza así un denominador estrictamente positivo en condiciones realistas y evita divisiones por cero. La corrección por curtosis se justifica por la teoría long-tail del deep learning: muestras raras o atípicas tienen influencia desproporcionada y producen distribuciones leptocúrticas que aumentan la curtosis, reduciendo GWA y señalando patrones de aprendizaje problemáticos.
...
**Probe set.** Subconjunto estratificado por clase con tamaño $M = 512$ o $M = 1024$ ejemplos. La estratificación garantiza cobertura uniforme y reduce varianza del estimador de curtosis, que es sensible a desbalance.

**Claves de logging.** Para cada época se persisten al menos cuatro entradas: `gwa/score_mean` (la media $\mathbb{E}[\text{score}]$), `gwa/kurt` (el exceso de curtosis del numerador, antes de sumar $\beta$), `gwa/value` (el escalar $\text{GWA}_T$ resultante) y `gwa/per_sample_hist` (histograma de los scores per-sample para análisis de outliers y muestras mal etiquetadas).
...
def gwa_epoch(accumulator, beta=1.2, eps=1e-12):
    ...
    excess_kurt = m4 / (m2.pow(2) + eps) - 3.0
    gwa = m1 / max(excess_kurt + beta, eps)
    return {"gwa/score_mean": m1.item(),
            "gwa/kurt": excess_kurt.item(),
            "gwa/value": gwa.item(),
            "gwa/per_sample_hist": accumulator.histogram()}
```
