...
**Entradas.** Gradientes calculados en el mismo $w$ sobre $M$ mini-batches muestreados i.i.d. del conjunto de entrenamiento. El paper utiliza **$M = 100$ gradientes per-sample (batches) al final de cada epoch**, formando después los $\binom{M}{2}$ pares (4950 para $M = 100$); el paper reporta del orden de 100 pares muestreados por epoch (conteo exacto a verificar contra el PDF). En el TFG se admite también la versión per-sample (un gradiente por ejemplo del probe set) y se recorta $M$ a $50$ por presupuesto de memoria.
...
- **Recorte de $M$.** El paper usa $M = 100$ gradientes per-sample (batches) al final de cada época, de los que forma los $\binom{M}{2}$ pares; el TFG recorta a $M = 50$ por presupuesto de memoria ($M\cdot P$). Cadencia `epoch` dentro de la ventana temprana (5/10/25/50% de épocas).
