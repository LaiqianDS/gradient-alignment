...
Las métricas reportadas son los **valores de gradient confusion** (aproximados como el mínimo y la densidad de las similitudes coseno por pares), el **training loss** a lo largo de los 200 epochs, la **test accuracy final**, comparativas a distintas **profundidades y anchuras**, y ablations con **batch normalization on/off** y **skip connections on/off**. El protocolo exacto (Sec. 7, p. 7) muestrea **100 pares de mini-batches de tamaño 128** al final de cada epoch, calcula el gradiente de cada mini-batch y los cosenos por par, es decir, 100 cosenos por época (no $\binom{100}{2}$ pares de un pool de 100 gradientes, como afirmaban versiones previas de estas notas). Las figuras 2, 3 y 4 sintetizan estos resultados sobre CIFAR-10.
...
**Entradas.** Gradientes calculados en el mismo $w$. El protocolo del paper (Sec. 7): **100 pares de mini-batches de tamaño 128** muestreados al final de cada epoch → 100 cosenos por época, sobre gradientes **per-minibatch**. La v1 del TFG implementa en cambio la versión **per-sample** (un gradiente por ejemplo del probe compartido, $M = 256$) y forma **todos** los $\binom{M}{2}$ pares. Son más pares que el paper, no un recorte, pero de otra distribución: por la Sec. 8 ($\eta \propto 1/B^2$ con gradientes promediados) los cosenos per-sample tienen colas más pesadas y **no son comparables** con las figuras 2–4 del paper.

**Cuándo computar.** Una medición por epoch, manteniendo $w$ fijo durante la medición. La métrica es esencialmente **global** sobre el vector de parámetros aplanado; opcionalmente puede desagregarse **por capa** para localizar la fuente de confusión, lo que resulta especialmente útil al estudiar el efecto de batch norm o de las skip connections en bloques profundos.

**Coste.** $M$ gradientes per-sample sin actualización, más $M^2$ productos internos baratos una vez formada la gram. El coste de cómputo es **moderado** y en la v1 se amortiza porque el barrido per-sample es único y compartido (`primitives.stream_shared`) con las otras cinco métricas que lo consumen. El cuello de botella es la memoria $M\cdot P$: para ResNet-18 fp32 con $M = 256$ son unos $12$ GB, y por eso `primitives.stream_gram` recorre el probe por bloques de filas, deja la matriz completa en RAM de host y acumula $G G^\top$ por bloques de columnas.
...
- **Decisiones de implementación.** Gradiente **bruto** $\nabla L$ (no la update preacondicionada de Adam), `model.eval()` durante la medición (congelar BN/Dropout), muestreo de batches **disjuntos** sin reemplazo. Comparte el **barrido per-sample** $\nabla L$ (`primitives.stream_shared`) con las otras cinco métricas que lo consumen, sobre el mismo probe congelado de $M = 256$.
- **Pitfall principal.** Memoria $M\cdot P$: en ResNet-18 fp32 con $M = 256$ son unos $12$ GB. La v1 lo resuelve con `stream_gram`, que recorre el probe por bloques de filas (pico de dispositivo $[\texttt{chunk\_size}, P]$), mantiene la matriz completa en RAM de host y acumula $G G^\top$ por bloques de columnas. NO se implementan las cotas teóricas (Teoremas 3.1–6.1) ni el OSGR-style; solo el estimador empírico. Los **ablations de overparameterización** (anchura↓confusión, profundidad↑confusión, BN+skip↓confusión) son material de discusión teórica, no se reproducen como experimento propio.
