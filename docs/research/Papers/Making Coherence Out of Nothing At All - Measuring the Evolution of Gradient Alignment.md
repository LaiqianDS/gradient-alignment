...
El algoritmo eficiente explota la identidad $\mathbb{E}[g_z \cdot g] = \mathbb{E}_z[g_z]\cdot\mathbb{E}_z[g_z] = \|g\|^2$: basta mantener dos sumas corrientes —el gradiente acumulado $g$ y la suma de cuadrados $\sum\|g_z\|^2$— sobre el sample, sin necesidad de almacenar los gradientes por ejemplo, lo que habilita un cómputo en streaming. El contraste con stiffness y gradient confusion es explícito: las no-linealidades de aquellas métricas (signo, normalización por norma, mínimo) y la exclusión de la diagonal $z = z'$ hacen difícil ligar su valor al cambio real de la pérdida tras un paso, mientras que $\alpha$ se conecta exactamente con la reducción de pérdida prevista por la expansión de Taylor.

### Datasets y modelos

Los experimentos principales se realizan sobre **ImageNet** con tres niveles de ruido en etiquetas (0%, 50% y 100% de etiquetas aleatorizadas). Las arquitecturas son **ResNet-18** como experimento central e **Inception-V3** como validación de generalidad. La configuración usa SGD con momentum $0.9$, batch size $4096$, el schedule de learning rate de Goyal et al. (2017), sin augmentation ni weight decay (para observar memorización en tiempos razonables) y un tamaño de muestra $m = 40\,356$ ejemplos de entrenamiento fijos. El apéndice añade experimentos con 25% y 75% de ruido y con $m$ duplicado a $80\,072$. Las comparaciones con Fort et al. (2019) sobre MNIST, Fashion-MNIST, CIFAR-10/100 y MNLI, y con Sankararaman et al. (2019) sobre CIFAR-10/100, se discuten cualitativamente.
