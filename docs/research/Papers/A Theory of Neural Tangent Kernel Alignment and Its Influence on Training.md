...
El punto de partida es la formulación del NTK. Para una red $f(\boldsymbol{x}, \boldsymbol{\theta})$ entrenada por descenso por gradiente sobre una pérdida $\mathcal{L} = \sum_\mu \ell(f(\boldsymbol{x}^\mu, \boldsymbol{\theta}), y^\mu)$ con $P$ ejemplos, la dinámica de los parámetros es
...
donde el **NTK** se define por $K(\boldsymbol{x}, \boldsymbol{x}'; \boldsymbol{\theta}) = \frac{\partial f(\boldsymbol{x}, \boldsymbol{\theta})}{\partial \boldsymbol{\theta}} \cdot \frac{\partial f(\boldsymbol{x}', \boldsymbol{\theta})}{\partial \boldsymbol{\theta}}$. Sobre el conjunto de entrenamiento el NTK queda completamente descrito por la matriz de Gram $\boldsymbol{K}(\boldsymbol{\theta}) \in \mathbb{R}^{P \times P}$. Para medir cuánto se parece este operador a la tarea, los autores adoptan la métrica de alineación de Cortes et al. (2012):
...
- **Pitfalls / decisiones:** $K=JJ^\top$ PSD por construcción; $Y$ one-vs-rest $\pm 1$ (no one-hot); forma sin centrar como en Shan & Bordelon (Cortes original sí centra). KSM multiclase off-by-default por coste $C\times$.
