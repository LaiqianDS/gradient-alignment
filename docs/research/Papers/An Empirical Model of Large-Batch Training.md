...
La verificación empírica cubre ocho tareas distribuidas en tres paradigmas de aprendizaje. En image classification se estudian MNIST (CNN simple con SGD), SVHN (CNN simple con SGD y Adam, junto con un autoencoder y un VAE para generative modeling), CIFAR-10 (ResNet-32 con momentum) e ImageNet (ResNet-50 con momentum y schedule de learning rate decay). En reinforcement learning aparecen siete juegos de Atari (Alien, Beam Rider, Breakout, Pong, Qbert, Seaquest, Space Invaders) entrenados con A2C y RMSProp, además de los agentes de Dota 1v1 y 5v5 entrenados con PPO asíncrono por el equipo OpenAI Dota. En language modeling utilizan un LSTM autoregresivo de tamaño 2048 (también 1024 y 512 para estudiar la dependencia con el tamaño del modelo) sobre el Billion Word Benchmark con codificación BPE de vocabulario 40k, optimizado con Adam. El bloque de generative modeling se completa con un Variational Autoencoder de arquitectura InfoGAN sobre SVHN y un autoencoder simple equivalente.
...
**Claves de logging.** `noise_scale/simple` como escalar por época, `noise_scale/noise` como variante exacta cuando se mide (en validación puntual), y `noise_scale/per_layer/{name}` como diccionario por módulo. Se conservan también las normas intermedias $\sum_i \|g_i\|^2/B$ y $\|G\|^2$ (y, si se usan, $\|G_{\text{small}}\|^2$ y $\|G_{\text{big}}\|^2$) para auditoría posterior y recálculo con distintas ventanas de EMA.
