...
donde $\Sigma$ es la covarianza por ejemplo del gradiente estocástico y $G$ el gradiente verdadero (esperanza sobre el dataset). La versión exacta $\mathcal{B}_{\text{noise}} = \mathrm{tr}(H\Sigma)/(G^\top H G)$ se reserva como referencia teórica, porque requiere productos Hessiano-vector y se descarta por coste.
...
**Granularidad.** Una medición por época durante toda la corrida. La trayectoria de $\mathcal{B}_{\text{simple}}$ es más informativa en la ventana temprana. La frecuencia por época basta para capturar el crecimiento monótono observado en el paper (uno o dos órdenes de magnitud a lo largo del entrenamiento).
