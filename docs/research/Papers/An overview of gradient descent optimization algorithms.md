...
con valores por defecto $\beta_1 = 0.9$, $\beta_2 = 0.999$, $\varepsilon = 10^{-8}$. Es el método por defecto cuando se quiere convergencia rápida en redes profundas o complejas con poco esfuerzo de tuning. **AdaMax** generaliza $v_t$ a la norma $\ell_p$ y, llevándolo al límite $\ell_\infty$, define $u_t = \max(\beta_2 \cdot u_{t-1},\, |g_t|)$, $\theta_{t+1} = \theta_t - (\eta / u_t)\, \hat{m}_t$, con valores por defecto $\eta = 0.002$, $\beta_1 = 0.9$, $\beta_2 = 0.999$; resulta útil cuando se desea acotar naturalmente la magnitud del paso por parámetro. **Nadam** (Dozat, 2016) incorpora el momento de Nesterov dentro de Adam reemplazando $\hat{m}_{t-1}$ por $\hat{m}_t$ en la regla expandida,
