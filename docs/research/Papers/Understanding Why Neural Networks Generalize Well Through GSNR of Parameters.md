...
**Coste.** Gradientes per-sample sobre $M = 256$ con varianza por parámetro vía `vmap`; el coste dominante es una pasada *forward-backward* vectorizada equivalente a un minibatch de tamaño $M$ pero con tensor de gradientes de dimensión $(M, P)$. La v1 **no materializa** esa matriz $[M, P]$: `primitives.stream_grad_moments` recorre el probe en bloques de filas y acumula en float64 los momentos por columna $S_j = \sum_i g_{i,j}$ y $Q_j = \sum_i g_{i,j}^2$, así que el coste de memoria es $\mathcal{O}(P)$ y el pico del dispositivo es $[\texttt{chunk\_size}, P]$. El *probe set* es el mismo que usan `m_coherence` y `stiffness`, y el barrido per-sample se hace **una sola vez** por medición para las seis métricas que lo consumen (`primitives.stream_shared`).
...
- **Cómo se usa.** Estimación per-sample sobre el `probe_set` fijo compartido ($M = 256$, sin materializar la matriz $[M, P]$: momentos $S_j$/$Q_j$ en streaming por bloques de filas); agregación por **mean** sobre parámetros ($\text{GSNR}_{\text{global}} = \operatorname{mean}_j r_j$), medida en ventanas tempranas (5/10/25/50% de épocas) y correlacionada con generalización y eficiencia. Usa el mismo probe que `m_coherence` y `stiffness`, y el barrido per-sample es único y compartido entre las seis métricas que lo consumen.
