...
**Métrica concreta.** El paper define dos estadísticos que conviene tratar por separado. La **Average Variance** es la traza normalizada de la matriz de covarianza del estimador del gradiente medio, $\overline{\mathbb{V}} = \frac{1}{d}\sum_{j=1}^d \mathbb{V}[g_j]$, una varianza absoluta agregada coordenada a coordenada. La **Normalized Variance** del paper es, literalmente, el cociente por coordenada $\mathbb{V}[g_j] / \mathbb{E}[g_j^2]$, promediado sobre coordenadas y acotado en torno a 1 (por eso satura en ImageNet en las Figs. 3d–3f). El denominador es el segundo momento **no central** (p. 5, §4: *"divided by its second non-central moment"*). La forma que el TFG adopta como NGV, $\text{tr}(\text{Cov}(g)) / \|\mathbb{E}[g]\|^2$ (estilo McCandlish), es una **adaptación deliberada**: ambas están relacionadas monótonamente ($NV = \text{NGV}/(1+\text{NGV})$ bajo agregación suma/suma), de modo que se conservan los rankings. El propio paper es internamente inconsistente: escribe $V[g]/E[g^2]$ pero glosa "(normalized gradient larger than one)". Esto explica la confusión de versiones previas de estas notas. La varianza normalizada, y no la absoluta, es la que el paper destaca como comparable entre problemas de distinta escala.
...
**Coste y memoria.** Coste de cómputo: $K$ pasadas backward por medición, sin paso del optimizer; con el $K = 10$ del pipeline y una medición por época en CIFAR-10 el overhead sobre el coste total de entrenamiento es de unos pocos puntos porcentuales. Memoria: con acumuladores streaming basta con almacenar dos buffers del tamaño del modelo (suma de gradientes y suma de cuadrados); orden de magnitud $\mathcal{O}(d)$ floats, donde $d$ es el número de parámetros. Guardar los $K$ vectores de gradiente completos cuesta $K \cdot d$ floats, unos $0{,}47$ GB en ResNet-18 fp32 con $K = 10$; es lo que hace la v1 (pila $[K, P]$), y el streaming $S$/$Q$ queda como optimización.
...
- `var/per_layer/{name}` — NGV por tensor de parámetros, donde `{name}` es el nombre devuelto por `named_parameters()`, esto es, una entrada por tensor de peso/bias (`layer1.0.conv1.weight`, `fc.bias`, etc.), no una por módulo. Queda para v2: la v1 no la emite, igual que ninguna otra métrica del registro emite claves `per_layer`.
...
def measure_ngv(model, loss_fn, sampler, K=10, eps=1e-12):
...
**Implementación:** $K = 10$ gradientes de sub-batch del probe (la v1 materializa la pila $[K, P]$; el streaming $S$/$Q$ queda como optimización). Coste bajo (≈3–5% overhead). El estimador plug-in satura en ≈K (ver Avisos).
