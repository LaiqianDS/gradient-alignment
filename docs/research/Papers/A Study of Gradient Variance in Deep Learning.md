...
**Métrica concreta.** El paper define dos estadísticos que conviene tratar por separado. La **Average Variance** es la traza normalizada de la matriz de covarianza del estimador del gradiente medio, $\overline{\mathbb{V}} = \frac{1}{d}\sum_{j=1}^d \mathbb{V}[g_j]$, una varianza absoluta agregada coordenada a coordenada. La **Normalized Variance** o NGV es el cociente $\mathbb{V}[g] / \mathbb{E}[g]^2$, interpretado como el inverso de un signal-to-noise ratio. En la práctica se reporta el escalar agregado $\text{NGV} = \text{tr}(\text{Cov}(g)) / \|\mathbb{E}[g]\|^2$, equivalente a la traza de la covarianza dividida por la norma cuadrada del gradiente medio. La NGV, y no la varianza absoluta, es la que el paper destaca como predictor de la velocidad de progreso de SGD entre problemas de distinta escala.

**Entradas.** Para un peso $w_t$ fijo se muestrean $K = 20$–$40$ mini-batches independientes (cada uno del mismo tamaño $B$ usado en entrenamiento) y se calculan los gradientes $g_1, \dots, g_K$. La estadística se computa **coordenada a coordenada**: para cada parámetro $j$ se acumulan $\mathbb{E}[g_j]$ y $\mathbb{V}[g_j]$ sobre los $K$ batches, y luego se agrega (suma sobre $j$) para obtener el escalar global. El paper recomienda computar también la versión **por capa**, lo que permite localizar la fuente del ruido cuando ciertas capas dominan la covarianza.

**Rangos típicos e interpretación.** La Average Variance, al ser absoluta, no es comparable entre escalas: $\sim 10^{-4}$ en CIFAR-10, $< 10^{-6}$ en ImageNet, $\sim 10^{-8}$ en MNIST tardío. La NGV sí es comparable: valores por debajo de 1 indican que la señal del gradiente medio domina sobre el ruido, valores por encima de 1 que el ruido domina y un paso individual de SGD es esencialmente direccional-ruido. Una NGV próxima a 1 es la frontera donde el SNR cae a la unidad y donde, esperablemente, aumentar el batch size deja de compensar (es la región conectada con el *gradient noise scale* de McCandlish et al.).
...
**Implementación:** $K = 20$–$40$ gradientes por batch, streaming $S = \sum_k g_k$ y $Q = \sum_k \|g_k\|^2$. Coste bajo (≈3–5% overhead). Comparte batch-grad sweep con otras métricas de la familia varianza, ver `metrics_impl.md §2.1`.
