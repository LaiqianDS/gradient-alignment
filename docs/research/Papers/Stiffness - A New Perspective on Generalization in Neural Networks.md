...
En segundo lugar, la stiffness es sensible al contenido semántico. En CIFAR-100 la matriz de class stiffness exhibe estructura de *coarse-grain* alineada con super-clases (grupos de cinco clases semánticamente conectadas, como los mamíferos pequeños o los vehículos terrestres) e incluso super-super-clases (la distinción living/non-living). Esto demuestra que la red captura jerarquías semánticas más allá del nivel de etiqueta usado en el entrenamiento. Como hipótesis ilustrativa, cabría esperar que entradas tipo $C(\text{leopard}, \text{tiger})$ resulten marcadamente más altas que $C(\text{leopard}, \text{truck})$, sin que el modelo haya visto nunca la etiqueta "felino". El paper solo muestra la estructura agregada por super-clase en las Figs. 6–8, nunca esas celdas concretas.

En tercer lugar, la dynamic critical length $\xi$ depende sistemáticamente del learning rate. Los learning rates mayores producen $\xi$ menores, es decir, funciones aprendidas localmente más maleables (menor stiffness) y más fáciles de doblar mediante actualizaciones de gradiente, incluso cuando logran accuracy comparable. Este hallazgo apunta a un rol regularizador del learning rate sobre el *tipo* de función aprendida y no solo sobre la velocidad de convergencia.
...
La variante de signo es más informativa para la stiffness *between-class*, mientras que la de coseno es preferible para la *within-class*. Siguiendo el paper, se distinguen ambas. La within-class restringe la esperanza a pares con $y_i = y_j$ y captura generalización intra-clase; between-class restringe a $y_i \neq y_j$ y mide transferencia entre clases.

**Entradas.** La métrica consume gradientes per-sample $\nabla_W \ell(x_k; W)$ calculados sobre un *probe set* fijo y disjunto del batch de entrenamiento. En la práctica del TFG se usará un probe estratificado por clase, muestreado una sola vez al inicio del run y reutilizado en cada medición para que la varianza temporal refleje cambios reales en el modelo y no resampling. Tamaños sugeridos en línea con el paper: $N \approx 500$ ejemplos para datasets de 10 clases (MNIST, Fashion-MNIST, CIFAR-10). Para el TFG el default operativo es $M = 256$, un único probe congelado (`config.probe_size`, dentro de `FIXED_KNOBS`) del que salen los per-sample grads de todas las métricas per-sample, `m_coherence` y `gsnr` incluidas.

**Granularidad temporal.** Una medición por época durante toda la corrida. La señal es más predictiva en la fase inicial (las primeras épocas), que es donde Fort et al. la muestran más fuerte. Para reporting se loguea en los hitos 5%, 10%, 25%, 50% y 100% de épocas, que es la convención del comparador de generalización del TFG.

**Granularidad estructural.** El plan es computar en ResNet-18 la stiffness *last-layer-only* ($P_{\text{eff}} \approx 5\text{k}$ frente a $11.7\text{M}$ del modelo completo, consistente con `gwa`) y *full-parameter* en MLP y CNN pequeña. Fort et al. reportan stiffness por bloques, lo que avala la restricción. **La v1 no lo implementa**: computa full-parameter en todos los modelos (la primitiva `named_last_linear` existe pero `stiffness.py` no la usa), decisión a resolver antes del pilot por la presión de memoria descrita abajo. La matriz $C(c_a, c_b)$ de class stiffness queda igualmente fuera de scope v1 (solo se emiten los 6 escalares).
...
En cuanto a interpretación de la señal, $\bar S_{\cos}^{\text{within}}$ alto es mejor (clases con features compartidos y SGD que transfiere mejoras intra-clase), $\bar S_{\cos}^{\text{between}} \approx 0$ es esperable, y se reportan también los promedios globales `cos` y `sign`. El cuello operativo es la gram $GG^\top$, con coste de memoria $M \cdot P$ floats que en ResNet-18 fp32 con $M=256$ asciende a $\sim 12\,\text{GB}$. La v1 lo mitiga con streaming: `primitives.stream_gram` recorre el probe en bloques de filas, así que el dispositivo solo sostiene $[\texttt{chunk\_size}, P]$, la matriz $[M, P]$ completa vive en RAM de host y el producto se acumula por bloques de columnas. No se restringe a la última capa: `stiffness.py` llama a `stream_gram` sobre todos los parámetros, en todos los modelos. El barrido per-sample $\nabla L$ es único y compartido (`primitives.stream_shared`) con las otras cinco métricas per-sample, sobre el mismo probe congelado de $M = 256$. No se implementa "stiffness vs distancia" ni la dynamic critical length $\xi$.

## Discrepancias detectadas

Se documentan tres discrepancias menores entre el paper y el resumen [[Métricas]], todas marginales:

La primera es de notación. El paper usa $|\vec{g}|$ para la norma euclídea, mientras que en este archivo y en [[Métricas]] se ha unificado a $\|\vec{g}\|$, más estándar. No hay impacto matemático.

La segunda concierne al modo de muestreo *train-val*. El paper afirma que train-train, train-val y val-val se comportan de forma muy similar ("all 3 options are remarkably similar") y de hecho reporta los **tres regímenes lado a lado** en las Figs. 3, 4, 5, 11 y 12 (no "mayoritariamente train-train", como decían versiones previas de estas notas). En el TFG se sigue una elección equivalente —probe estratificado disjunto del batch de training—, pero se etiqueta como "val-val" cuando el probe proviene del split de validación. La métrica numérica es la misma; conviene dejar claro en el código qué split se está usando para el probe.
