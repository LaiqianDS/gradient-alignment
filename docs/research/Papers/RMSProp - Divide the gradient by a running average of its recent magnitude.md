...
El único hiperparámetro explícito en la slide 6e es el factor de suavizado $0.9$ sobre el segundo momento no centrado (es decir, vida media corta sobre los últimos gradientes cuadrados); el *learning rate* global solo aparece implícito en la proporcionalidad del update. No se introduce explícitamente término de estabilidad numérica $\epsilon$ ni corrección de sesgo del estimador; esos detalles aparecerán formalizados después en Adam. La cita en la slide es literal: *"Dividing the gradient by sqrt(MeanSquare(w,t)) makes the learning work much better (Tijmen Tieleman, unpublished)"*. La evolución típica de $\text{MeanSquare}(w,t)$ refleja con vida media corta el régimen reciente de gradientes. Si el gradiente sobre $w$ ha sido grande en los últimos pasos, el divisor crece y reduce el paso efectivo; si el gradiente decrece, el divisor también lo hace gradualmente y restituye un paso mayor. Esta dinámica contrasta con AdaGrad (Duchi et al., 2011), donde el divisor acumula $\sum_{s\le t} g_s^2$ sin olvido, lo que provoca que el *learning rate* efectivo decaiga monotónicamente y termine paralizando el entrenamiento en horizontes largos. RMSProp sustituye esa suma por una EMA, recuperando capacidad de adaptación incluso tras muchos pasos. Como extensiones discutidas en *Further developments of rmsprop*, Hinton sugiere combinar RMSProp con momentum estándar (señalando que ayuda menos de lo habitual y requiere más investigación), con momentum de Nesterov (Sutskever, 2012) y con *learning rates* adaptativos por conexión al estilo Jacobs. Con Nesterov funciona mejor si el RMS de los gradientes recientes se utiliza para dividir la corrección y no el salto en la dirección del gradiente acumulado. Como método relacionado se menciona el de Yann LeCun en *No more pesky learning rates*.
...
**Entradas.** Si se incluyera RMSProp como optimizador opcional, las cantidades que el TFG observa son las que el propio optimizador mantiene internamente: la media móvil del cuadrado del gradiente $\text{MeanSquare}(w,t)$ por parámetro, accesible en PyTorch como `optimizer.state[p]['square_avg']` para cada parámetro `p` de `torch.optim.RMSprop`. A partir de ella se derivan dos magnitudes diagnósticas: el *learning rate* efectivo por parámetro $\eta_{\text{eff}} = \eta/\sqrt{\text{MeanSquare}+\epsilon}$ y la razón $g/\sqrt{\text{MeanSquare}}$, una señal tipo SNR que indica cuántas desviaciones cuadráticas recientes vale el gradiente actual. Nótese que `torch.optim.RMSprop` suma $\epsilon$ **fuera** de la raíz, $\sqrt{\text{MeanSquare}}+\epsilon$, igual que Adam tanto en el paper (Alg. 1: $\hat m/(\sqrt{\hat v}+\epsilon)$) como en PyTorch; el contraste real es con TensorFlow-RMSprop o la ec. 18 de Ruder, que usan $\sqrt{v+\epsilon}$. Estas magnitudes no requieren *forward*/*backward* adicionales: se leen del *state* del optimizador después de `optimizer.step()`.
...
def log_rmsprop_state(optimizer, model, step, logger, eps=1e-8):
    ...
    for name, p in model.named_parameters():
        ...
        rms = ms.sqrt().add(eps)              # sqrt(MS) + eps, como torch.optim.RMSprop
...
- **Rol: related work (junto a Adam), no métrica del registry.** Precursor histórico del optimizador adaptativo; no aporta ninguna de las 8 métricas + baseline. No entra tampoco en el *sweep* cerrado (SGD y Adam): equivaldría a un caso intermedio (segundo momento sin momentum), sin contraste cualitativo nuevo.
