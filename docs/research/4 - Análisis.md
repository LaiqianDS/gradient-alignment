...
Esto es una decisión del preregistro y no solo de ejecución, por un motivo concreto: descartar métricas antes de medirlas equivale a elegir qué hipótesis se pueden contrastar según el presupuesto de cómputo en vez de según la pregunta de investigación. H1 y H3 comparan métricas entre sí, así que quedarían apoyadas en un conjunto recortado por razones ajenas a la ciencia. El coste medido sobre la matriz es de ~2,04x el wall-clock de un run sin instrumentar en el peor caso hasta ahora (fc × cifar10 × sgd), dentro de la cota <3-4x fijada de antemano. [Enmiendas 2026-08-05 y 2026-08-08, ver §Historial de revisiones; la cifra de 3,21x que citaban las versiones anteriores es del pilot, anterior al barrido compartido, y sobrestima el coste del código que corre la matriz.] La **poda** de métricas es posterior y solo afecta a lo que la memoria destaca, nunca a lo que se contrasta.
...
**De dónde sale δ_H2 = 0,15.** El margen no viene de la tabla de Cohen sino de la pregunta de la tesis, que es si instrumentar el gradiente compensa. El coste medido es de ~2,04x el wall-clock de un run sin instrumentar en el peor caso (§Unidad de análisis), así que la métrica tiene que aportar información suficiente para justificar duplicar el presupuesto de cómputo frente a un baseline que es gratis. [Enmiendas 2026-08-05 y 2026-08-08, ver §Historial de revisiones; el margen no se mueve con la cifra, ver allí el motivo.] Una parcial cross-celda por debajo de 0,15 explica menos del 2,3% de la varianza de rangos residual, y a ese precio eso es irrelevante para la decisión de cualquier practicante.
...
**Colapso sin divergencia** (enmienda 2026-08-08, anterior a calcular ninguna correlación entre métrica y VD). La regla anterior decide con la finitud de la fila, y hay un caso que se le escapa por ser perfectamente finito. Un run puede quedarse con la loss clavada en ln(C) y la accuracy en el azar sin producir un solo NaN: las ReLU mueren, la última capa recibe una entrada exactamente nula y el clasificador solo emite su sesgo. En los primeros 268 runs de la matriz ocurre en **26**, todos en learning rates altos de `cnn × mnist` y `fc × mnist × adam`, frente a 11 divergencias con NaN.

El alcance es estrecho y conviene ser preciso. Los gradientes por muestra **no** son nulos (`var/avg` > 0 en todas esas épocas), de modo que casi todas las columnas son mediciones reales de una geometría degenerada y se quedan como están; incluso `stiffness/cos_within` = 1,0 exacto es real, porque el gradiente del sesgo es literalmente el mismo vector para todas las muestras de una clase. La única cantidad fabricada es **`gwa/score_mean`**: GWA usa el gradiente del peso de la última capa, que aquí sí es exactamente cero, y su norma se protege con `clamp_min(EPS)`, así que el coseno sale 0/EPS = 0,0 exacto. El coseno de un vector nulo no existe.

**Regla: un `gwa/score_mean` exactamente igual a 0,0 se marca como faltante para GWA en ese (run, época)**, igual que las columnas de signo del párrafo anterior. Discrimina sin ambigüedad sobre los datos existentes: los 22 runs que lo cumplen están colapsados y ninguno de los 242 sanos lo cumple. `gwa/kurt` y `gwa/value` ya salen NaN por la rama de varianza nula del propio código.

Los runs colapsados **no se excluyen** por lo demás. Un run que colapsa es genuinamente ineficiente, y una métrica que lo anticipe en la ventana temprana es genuinamente útil: es la misma lógica por la que los censurados entran con el peor rango en vez de tirarse. La regla se aplica en el análisis sobre lo ya registrado, así que no obliga a re-correr nada.
...
*Observación del 2026-08-08, no es un cambio de regla.* La anticipación se queda corta en dos celdas. `fc × cifar10 × sgd` sale censurada al 100% (26 de 26 runs terminados; techo de 0,584 frente a un umbral de 0,65), y no por presupuesto: el pilot corrió esa configuración al presupuesto doblado y tocó el mismo techo en la época 40 para quedarse plano después. Es el techo de la arquitectura, así que `fc × cifar10 × adam` caerá igual. La proyección pasa de 4 celdas degeneradas a **6**, lo que deja VD1 con **18 celdas elegibles**, exactamente el suelo de §Matriz incompleta. El umbral **no se toca**: moverlo después de ver que una celda no lo alcanza degradaría VD1 a exploratoria por la propia política de enmiendas. Queda anotado que la restricción es del preregistro y no del dato, porque `epochs_to_threshold` se recalcula desde `trajectory.parquet` sin coste de cómputo (verificado, 268 de 268 coinciden con `summary.json`).
...
- **2026-08-08 (enmienda, con la matriz en marcha y antes de calcular ninguna correlación).** Primera revisión con datos reales de matriz: 268 runs terminados, MNIST completo más media celda de `fc × cifar10 × sgd`. Se han mirado tiempos, curvas de validación y columnas de métrica; **no** se ha calculado ninguna asociación entre métrica y VD, que es lo que la política de enmiendas protege. Tres cambios. (1) **Corrección factual del coste**, que enmienda a su vez la del 2026-08-05: el 3,21x procedía del pilot, ejecutado el 2026-06-15, y el barrido compartido entró el 2026-06-17 (commit `8566fc3`), así que toda cifra de coste del vault era de una versión anterior del código. Medido sobre la matriz, el peor caso baja a **2,04x**, y en la misma celda la comparación directa da 3,09x → 2,04x. Que la causa es la optimización se comprueba con que el tiempo de entrenamiento por época no cambia (1,671 s frente a 1,638 s), el de medición se reduce a la mitad y las trayectorias salen idénticas bit a bit, que es justo lo que promete el invariante de equivalencia entre la ruta compartida y la independiente. **δ_H2 sigue en 0,15**, por el mismo motivo que en la enmienda anterior y ahora en la dirección contraria: el coste justifica el margen, no lo calcula, 2x sigue siendo alto, y bajarlo solo restaría potencia a la hipótesis decisiva. (2) **§Censura gana la regla de colapso sin divergencia**, un caso que la regla del lado predictor de 2026-07-25 no cubría porque decide con la finitud de la fila y estos runs son finitos. Se acota a lo que de verdad está fabricado, que es solo `gwa/score_mean` = 0,0 exacto sobre un gradiente de última capa nulo; el resto de columnas del run colapsado son mediciones reales de una geometría degenerada y se quedan. Se aplica en el análisis sobre lo ya registrado, sin re-correr nada y sin tocar `src/` con la matriz en marcha. (3) **Observación anotada en §Censura** de que la anticipación de celdas degeneradas se queda corta: `fc × cifar10` también lo es, la proyección sube a 6 celdas y VD1 queda con 18 elegibles, justo en el suelo. No es un cambio de regla y el umbral no se toca.
- **2026-08-05 (enmienda, corrección factual).** Se corrige el coste de instrumentación citado en §Predictores y en §Estadístico: el peor caso medido no es fc × tiny_imagenet a 2,08x sino **fc × cifar100 a 3,21x** el wall-clock de un run sin instrumentar, y cuatro runs de FC superan 3x. El error venía de leer el peor caso por coste **absoluto** de medición (Tiny tiene el mayor, 2153 s) en vez de por cociente contra su propio entrenamiento, que también es el más caro. **Esta enmienda no cambia ningún criterio.** δ_H2 sigue en 0,15: la cifra de coste es la justificación del margen, no el margen, y la corrección va en la dirección que lo refuerza, porque exigir una parcial de al menos 0,15 es si acaso más generoso cuando instrumentar cuesta el triple y no el doble. Tampoco degrada nada a exploratorio: es anterior a mirar cualquier resultado de matriz, que a esta fecha no existe. La cota <3-4x fijada de antemano sigue cubriendo el peor caso, aunque ahora cerca de su extremo inferior en lugar de holgadamente.
