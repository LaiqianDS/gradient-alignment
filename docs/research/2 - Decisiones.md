...
Qué entra en una entrada: la decisión, su porqué en corto, la evidencia con su número, y las trampas. Nada más. El registro se poda mientras se trabaja, y las decisiones y sus fechas no se podan nunca; la regla completa está en [[3 - Progreso]] §Reglas de trabajo.

## Pendientes (sin cerrar)

Bloquean experimentos.
...
#### La D de Somers pasa la auditoría de sus condiciones de uso con dos límites, y la memoria los declara

**Qué se decidió.** Cuatro cosas, a raíz de tres preguntas: si la D de Somers es válida para el estudio, si es fuerte y si se cumplen sus condiciones de uso. (1) La D se mantiene tal cual, estimador e intervalo, y el código no cambia: el estimador está bien aplicado y el intervalo se equivoca hacia el lado ancho salvo en un caso conocido. (2) Se declaran en la memoria dos límites que la auditoría sacó a la luz. El primero es la celda de Tiny-ImageNet con FC y Adam, velocidad por hitos al 5 %, donde siete predictores (`log_lr`, las tres lecturas de la referencia, GSNR, disparidad y GWA) dan D = ±1 con error típico exactamente cero y cuentan como seguras sin intervalo real. El segundo es que el censo de formas solo aparta las celdas donde la relación monótona es imposible (predictor monótono, VD no), y un predictor irregular no se aparta aunque su D pueda quedar corta. Van en `metodologia.tex` §Coeficiente por celda (los dos, como límite del intervalo y de la regla), `resultados.tex` §Existencia (con los números y lo que baja cada recuento sin la celda) y `conclusiones.tex` §Limitaciones (primera y segunda). §Coeficiente por celda cierra además con un párrafo que reúne las cuatro condiciones de la D y dónde se comprueba cada una, con la única frase nueva de la memoria: la censura es administrativa y el índice de Harrell estima la concordancia dentro del presupuesto. (3) No se cambia el nombre «D de Somers» por «D del predictor respecto a la variable», D(X|Y), que la revisión externa sugería con prioridad baja: la ecuación y la regla de empates de §Coeficiente por celda ya definen la variante (pares con VD distinta en el denominador, empates del predictor a cero) y la cita a Harrell la identifica. (4) Lo que no se hizo: H2, H4 y H5 no se recalcularon con el jackknife por seed; la revisión de estilo pasó solo por los pasajes nuevos; el recuento de celdas irregulares es solo al 5 %.

**Por qué.** Un intervalo de anchura cero deja fuera el cero de forma automática, y un recuento de celdas seguras que lo incluya sin decirlo está inflado en una celda. El censo de formas se presentaba como si cubriera todo el problema de la monotonía y solo cubre los casos imposibles; el sesgo restante va contra las métricas de gradiente, porque las tres lecturas de la referencia tienen la forma de la VD, y hay que decirlo donde se lee H2. Todo lo demás se cumple o está ya declarado, y esta entrada es el sitio donde vive la respuesta larga a «por qué la D» si el tribunal la pide.

**Validez y nombre.** Se cumple. La D solo pregunta, par a par, si el run con más predictor tiene más VD; no supone linealidad ni normalidad, solo relación monótona dentro de la celda. Lo que calcula `efficiency._pair_terms` es la variante asimétrica D(X|Y): el denominador son los pares con VD distinta y los empates del predictor cuentan cero pero siguen dentro; es exactamente la C de Harrell reescalada, C = (D + 1)/2, y coincide con tau sin empates ni censura. La prueba `test_somers_d_skips_outcome_ties_and_zeroes_predictor_ties` fija esa variante. Con los papeles cambiados, D(Y|X), el número es otro en cuanto hay empates.

**Censura.** Se cumple, y es el punto a favor que la memoria no dice con estas palabras: la crítica clásica a la C de Harrell (que depende del reparto de la censura, de ahí la C de Uno) no aplica, porque la censura es administrativa en un instante común. Sobre los 806: 573 cruzan y 233 quedan censurados; ningún cruce cae más allá del presupuesto; el presupuesto es el mismo para toda la celda; epochs de cruce distintas por celda, mediana 9 y mínimo 2 (Tiny-ImageNet FC Adam). La única excepción son dos runs de MNIST FC SGD con lr 0,3 (seeds 0 y 1) que aprendieron en la epoch 1 y divergieron en la 2: entran censurados por convención y no dejan medición al final.

**Pares comparables.** Se cumple al 5 % y falla en una celda al 10 %. Velocidad por hitos al 5 %: en riesgo mínimo 15 y mediana 30, pares mínimo 50 y mediana 384, ninguna celda vacía; sobre todos los runs, entre el 67 y el 91 % de los pares de cada celda son comparables (mediana 85 %). Al 10 %: en riesgo mínimo 9 y mediana 24, pares mediana 242, Tiny-ImageNet FC Adam sin ningún par y CIFAR-100 FC Adam con 8. Variables del final al 5 %: accuracy de test n mínimo 25 y mediana 32,5, pares mínimo 300 y mediana 509,5; gap n mínimo 20 y mediana 25, pares mínimo 190 y mediana 300.

**Monotonía.** A medias, y el protocolo solo cubre la parte segura. Formas al 5 % sobre las 24 celdas: VD1 valle en 20, accuracy de test pico en 17, gap pico en 13 (irregular en 6); val loss valle 21, val accuracy pico 20, TSE valle 19; disparidad pico 17; stiffness valle 8, baja 6, irregular 6; irregulares en la mayoría: escala de ruido 17, GSNR 12, GWA 11, confusión 11. Celdas apartadas en la familia primaria (velocidad/test/gap): stiffness 6/7/5, confusión 4/3/4, GSNR 4/3/4, escala de ruido 4/2/3, disparidad 2/2/3, GWA 2/2/3, TSE 2/3/1, val accuracy y val loss 1/2/1. Lo que el censo no aparta es lo demás: las cuatro métricas irregulares pueden dar una D baja sin aviso, y las tres lecturas de la referencia tienen la forma de la VD, así que dos curvas con el extremo en el mismo lr componen una relación monótona. La lectura de selección, que no compara órdenes, esquiva el sesgo en el accuracy de test.

**Independencia e intervalo.** No se cumple, y el intervalo sale ancho como dice la memoria. Reproducido el jackknife por seed de la entrada del 2026-09-04 sobre las 720 D de la familia primaria: error típico 0,58 veces el de quitar un run (cuartiles 0,38 y 0,82), menor en el 86 % de las 713 filas con error no nulo. Con t(4) = 2,78 las referencias llegan a 22 y 23 celdas seguras de 24 y las métricas ganan entre una y cinco (stiffness con velocidad 14 → 19, disparidad con test 16 → 20, `log_lr` con velocidad 11 → 20) o pierden (GWA con test 16 → 13, escala de ruido con gap 11 → 6, GWA con gap 10 → 8). La foto no cambia. El lado ancho es el seguro para afirmar asociación y el menos seguro para negarla.

**El caso del error cero.** En la celda: 25 aprendieron, 15 cruzan, y al cerrar la epoch 2 quedan en riesgo 15, los cinco de lr 3e-5 (cruzan todos en la epoch 3) y los diez de 1e-3 y 3e-3 (censurados); los de 1e-4 y 3e-4 ya habían cruzado. Los 50 pares comparables son todos entre los dos grupos, así que separar el lr más bajo de esos dos da D = ±1 y cada réplica de quitar un run vale lo mismo. Son las únicas siete filas con se = 0 de las 720 de la familia primaria. Sin la celda: GWA 16/1 → 15/1, GSNR 6/7 → 6/6, disparidad 4/9 → 4/8, posición 5/7 → 4/7, val loss 24/0 → 23/0, val accuracy 0/24 → 0/23, TSE 23/0 → 22/0; mediana de |D| de GWA 0,402 → 0,397; en H5 caen un par de acuerdo de GWA (6 → 5) y otro de la disparidad (3 → 2). Ningún veredicto se mueve. Es el learning rate leído de otra forma en su versión más pura: con dos grupos de lr en riesgo, separar uno del otro ya es ordenar todos los pares, y por eso siete predictores tan distintos dan el mismo valor.

**Fuerza, ventana del 5 %, mediana de |D| y celdas con intervalo fuera del cero sobre 24.** Velocidad por hitos: GWA 0,40 y 17, stiffness 0,35 y 14, GSNR 0,33 y 13, disparidad 0,28 y 13, `log_lr` 0,33 y 12; val accuracy y val loss 0,71 y 24, TSE 0,64 y 23. Accuracy de test: disparidad 0,34 y 16, GWA 0,29 y 16, confusión 0,18 y 10; val accuracy 0,40 y 18, val loss 0,39 y 19, TSE 0,33 y 17, `log_lr` 0,30 y 13. Gap: stiffness 0,39 y 16, disparidad 0,36 y 15, GSNR 0,33 y 13, GWA 0,21 y 10; val loss 0,54 y 22, val accuracy 0,53 y 22, TSE 0,51 y 23, `log_lr` 0,46 y 15. Error típico por run mediano 0,12 a 0,13. Con 40 runs una D nula tiene desviación típica 0,11, así que |D| por debajo de 0,2 es ruido. En pares, C = (D + 1)/2: una D de 0,40 son siete pares de cada diez bien ordenados y una de 0,71 más de ocho. Veredicto: moderada en dos o tres métricas, débil en el resto, nunca por encima de la referencia; «significativa» no es la palabra del estudio, porque no hay valor p por celda a propósito.

- **Trampa:** el cero de `d_stats` no es un fallo; con separación perfecta ninguna réplica de quitar un run puede cambiar la D. No parchearlo con un suelo artificial.
- **Trampa:** «irregular» en el censo es más de un cambio de sentido con tolerancia del 5 % del recorrido; con otra tolerancia el recuento cambia (entrada de las nubes, 2026-09-03).
- **Trampa:** la variante es D(X|Y), la de Harrell; quien recalcule con otra biblioteca tiene que comprobar cuál de las dos direcciones calcula, porque con empates dan números distintos.
- **Trampa:** la frase de Conclusiones «11 a 17 celdas de 24 en cuatro de las seis» es el censo al 5 % y no vale para otras ventanas.

#### Fuera el anexo de configuración y fuera todas las fechas de la memoria

**Qué se decidió.** (1) `anexo-configuracion.tex` se borra entero. Se van las versiones exactas de las librerías, las órdenes de ejecución, las claves de *commit* y la lista de los cuatro campos obsoletos de `summary.json`; que esos campos están obsoletos lo sigue diciendo Desarrollo. La máquina se rescata en un párrafo de Desarrollo §Arquitectura del sistema, porque es lo que hace interpretables los 14,2 GB y las 121,7 horas: una sola máquina con Linux, GPU RTX 5080 de 16 GB, sin clúster, y una línea de software sin números de versión. (2) Ninguna fecha se imprime ya en el cuerpo. Cae con ellas `metodologia.tex` §Cuándo se fijó, y la declaración de que el análisis es posterior a los datos, con las diez desviaciones y las dos citas, pasa a §Riesgos sin fechas; los siete punteros de cuatro capítulos apuntan ahí. (3) Antes, en el mismo carril: Metodología reagrupa las Tablas 4.1 y 4.2 por familias y bloques, §Calibración separa la regla del presupuesto de lo que de verdad se decidió, la tabla de calibración pierde la columna «Regla», §Población define las tres poblaciones y §El coeficiente de una celda dice por qué la D de Somers y no Spearman ni Kendall. En el Estado del arte, la Figura 2.1 se queda solo con lo que este trabajo usa.

**Por qué.** El anexo mezclaba lo que el lector necesita para entender una cifra del cuerpo con lo que solo sirve para reejecutar el código; lo primero cabe en un párrafo y lo segundo vive en el repositorio. Las fechas de decisión son del vault, no de la memoria, y §Cuándo se fijó era el único sitio que las imprimía.

**Evidencia.** Compila en 103 páginas de PDF (eran 112 antes de esta tanda), cero cajas desbordadas, cero referencias y cero citas sin resolver; barrido sobre los diez `.tex` con el patrón de fechas, ninguna fuera de comentarios. §Calibración baja de 1.710 a 1.615 palabras y el capítulo de 15.165 a 14.974.

- **Trampa:** «una sola máquina» no lo verifica nada en `reports/`, porque `config.yaml` guarda `device: auto` y el resumen no tiene clave de hardware. Es un dato aportado por el autor, no medido.
- **Trampa:** §Protocolo de análisis decía «las seis variables de eficiencia» y son siete, con la F1 macro. Corregido.

### 2026-09-05

#### El cierre por métrica: la tabla «Qué anticipa cada métrica» al final de Resultados, la respuesta en pares de cada diez y el techo de la escala de ruido; tres lecturas de anticipación se probaron y se descartaron

**Corregido el 2026-09-06: 255 no es un techo.** Es el valor al que tiende el estimador cuando el gradiente medio verdadero es cero, y la rejilla lo pasa. Medido sobre los 806, el 10,9 % de las 4.022 lecturas queda por encima, con máximo 3.067, y en la ventana del 5 % lo pasan 65 con máximo 498. Cae con ello la conversión r/(1 − r/256) y sus «unas 900» y «unas 10.000», que la memoria ya no lleva; siguen en pie la mediana de 166 y el 37,8 % por encima de 200. Los tres capítulos dicen ahora «valor de referencia».

**Qué se decidió.** Tres cambios en la memoria y ningún código. (1) `resultados.tex` §Conclusiones del capítulo cierra con la Tabla `tab:aportaciones`, seis filas, una por métrica que queda tras la poda, con qué lee, qué anticipa al 5 % y en cuántas celdas, dónde falla, y cómo queda frente a su artículo y frente a la validación; todas las cifras son las de las tablas de H1, H2, H4, H5 y H6, sin ningún número nuevo, y el párrafo que la presenta da la conversión de D a pares, (D + 1)/2. (2) En `conclusiones.tex` la respuesta a la pregunta se escribe en pares de cada diez: GWA alta al 5 %, cruce más tardío en siete pares de cada diez (D 0,40) y menos accuracy de test en seis y medio (0,29); stiffness alta, gap menor en siete (0,38), los mismos que la posición en la rejilla (0,46); el accuracy de validación, ocho y medio en velocidad (0,71) y siete en test (0,40). Solo entran relaciones con signo consistente entre celdas; el loss de validación con el gap se parte por arquitectura y no se traduce. (3) Una quinta salvedad en Resultados §Conclusiones del capítulo y una frase en Conclusiones §Limitaciones: el estimador de la escala de ruido, tr Σ̂ / ‖ḡ‖², tiende a M − 1 = 255 porque ‖ḡ‖² lleva tr Σ/M de sesgo al alza, y al 5 % sobre los 806 la mediana es 166, el 38 % queda por encima de 200 y el 9 % en 250 o más; una lectura r corresponde a una escala real r/(1 − r/256), 200 → unas 900 y 250 → unas 10.000. Medido el 2026-09-05 sobre `metrics_at_window.parquet` (ventana 0,05, `noise_scale/simple`, los que aprendieron), sin fichero en `results/`.

**Por qué.** Cada métrica estaba repartida entre seis secciones por hipótesis y no había ningún sitio donde leer qué hace cada una; la tabla no añade cifras y la memoria no cambia de página. El techo del estimador estaba en la tabla del Estado del arte como rango y en el vault desde el 2026-09-03, y ningún capítulo decía cuánta rejilla se aprieta contra él.

**Tres lecturas de «cuando X, anticipa Y» probadas ese día y descartadas, para no repetirlas.** (a) Muertos frente a lentos: al cerrar el 5 % hay 167 entrenamientos en el azar (val-accuracy por debajo de 1,25 veces el azar) y 14 aprenden después (ResNet-18 con lr 1,0 en los dos CIFAR, CNN con lr 3e-4 en CIFAR-100 y Tiny, y seis colapsados que se recuperan). La huella del colapso (`stiffness/cos_within` = 1,000, GWA sin valor por cosenos constantes, `gd/scalar` ≈ 0,19) está en 112 de los 114 colapsados no divergidos y el accuracy de validación no la ve; pero cinco de los 14 que aprenden la llevan también en la epoch 2 y se recuperan igual, y dentro de celda el loss de validación contra ln C ordena muertos y lentos mejor (D −0,86 sobre 57 pares) que la stiffness (−0,46) y GSNR (−0,61), igual que la posición en la rejilla (−0,86), porque los lentos están abajo y los muertos arriba. Juntando todo sin celdas, GWA definida 0,68, stiffness −0,66, disparidad 0,56 y loss/ln C −0,54, con errores típicos de 0,13 a 0,18. Los 154 muertos costaron 10,1 h de 121,7 y una regla en la epoch 2 habría ahorrado 7,4. Es la huella de la ReLU muerta que Desarrollo ya cuenta, no una predicción. (b) Divergencia: los 39 que divergen sin aprender lo hacen en la primera epoch, antes de cualquier medición; los dos de MNIST-FC-SGD con lr 0,3 que aprendieron divergen en la segunda. Nada que anticipar. (c) Escala absoluta entre celdas: juntando los 806 sin estratificar, GWA conserva su D con el cruce (−0,34 frente a −0,35 dentro de celda) y con la velocidad (0,30 frente a 0,35), las otras cinco caen por debajo de 0,16, y el loss relativo a ln C se queda en 0,51 (0,71 dentro de celda). La referencia gratuita gana también sin celdas. No queda código ni fichero en `results/` de estas tres cuentas; se registran aquí para no repetirlas.

**Evidencia.** Compila en 112 páginas de PDF, las mismas que antes de la tabla.

- **Trampa:** las medianas de D del capítulo son sobre signo, no en valor absoluto, y coinciden con |D| solo donde el signo es consistente; por eso GSNR con el gap (8/16 de signo crudo, |D| 0,32) no se traduce a pares en Conclusiones.
- **Trampa:** «un tercio de las lecturas» y «unas 900 / unas 10.000» viven en Resultados §Conclusiones del capítulo y en la fila de la escala de ruido de la tabla; Conclusiones solo repite «un tercio» y remite. Regla de la fase H: Conclusiones no lleva cifras que no estén en Resultados.
- **Trampa:** la conversión (D + 1)/2 es la C de Harrell, que el protocolo ya usa para el 0,8 (nueve de cada diez); con empates en el predictor cuenta medio par, y en velocidad va sobre los pares comparables por hitos.

#### Un análisis complementario del inicio del sobreajuste dentro de cada entrenamiento se hizo y se descartó el mismo día

**Qué se decidió.** Se calculó, fuera del protocolo, si tres marcadores leídos sobre la trayectoria de cada entrenamiento señalan el mínimo del loss de validación suavizado, el inicio del sobreajuste: el primer mínimo interior de la disparidad, el primer máximo interior de la stiffness intra-clase y el primer cruce de la coherencia por debajo de 1, con coincidencia en epochs, D de seguimiento dentro de celda y comparación con la parada temprana de paciencia 3 sobre la validación cruda. Se escribió como sección exploratoria de Resultados y se descartó entero, código, `results/` y memoria, porque no cambia ningún veredicto y ocupaba página y media en una memoria que se recorta; queda esta entrada para no repetirlo.

**Evidencia (2026-09-05, sobre los 642 de los 806 con mínimo interior).** Ninguna celda coincide con ningún marcador. La disparidad solo gira en 277 y lo hace 24 epochs después del mínimo de mediana, y en la ResNet-18 sigue bajando casi hasta el final; la stiffness tiene el máximo en la primera epoch en el 81 %, así que su caída empieza antes de la primera medición; la coherencia cruza 1 en 242, en cualquier momento, con seguimiento negativo. La paciencia sobre la validación cruda dispara en 612 y se adelanta al mínimo suavizado en 123. Lectura: el batch de medición es de entrenamiento y las métricas registran el ajuste de esos 256 ejemplos, que sigue después de que la validación toque fondo.

- **Trampa:** con una medición por epoch y sin lectura en la inicialización, la afirmación de Fort sobre la stiffness no se refuta, no se ve; repetirlo exige medir dentro de la primera epoch.
- **Trampa:** un marcador que dispara siempre en la epoch 4 «gana» a la paciencia cuando el mínimo cae antes; una lectura de utilidad sin coincidencia no vale nada.

### 2026-09-04

#### El código queda autocontenido: sin referencias al vault ni a la memoria, comentarios al mínimo y código muerto fuera

**Qué se decidió.** Ningún comentario, nombre ni impresión del código ni de sus README apunta ya a `docs/`, al vault, a la memoria, a una fase, a una hipótesis ni a una fecha; cada regla compartida vive en un solo módulo; los comentarios que repiten la línea siguiente salen y se quedan los porqués numéricos; el código muerto sale con sus pruebas y sus PDF; los duplicados se unifican. La figura `signos-h1` pasa a `signos`. Las citas de artículo en las definiciones de las métricas se quedan, porque son la definición.

**Qué no se tocó, y por qué.** El script de entrenamiento no cambia ni una línea, porque es el que produjo los 960 entrenamientos.

**Evidencia.** Las siete figuras regeneradas desde `reports/` salen idénticas byte a byte a las anteriores.
...
- **Trampa:** la nota del seminario (50 a 100 páginas) no dice si cuenta portada, índices, bibliografía y anexos. Resuelto el 2026-09-06: con el anexo de configuración fuera son 105 páginas de PDF y 88 impresas de cuerpo, por debajo de 100 cuente lo que cuente.
...
**Qué se decidió.** La regla del protocolo tal cual: por predicción que el artículo enuncia, celdas con el intervalo fuera del cero a favor del signo predicho y en contra, a una cola porque el sentido está fijado de antemano, con el mismo intervalo de todas las tablas y sin bajar la barra a 1,645, para que una celda signifique lo mismo en todo el capítulo (`results/signos.parquet`, con la cuenta repetida por arquitectura). Tres concreciones: la predicción de la m-coherencia sobre el gap cuenta como fuerte, porque el vault la verificó contra el PDF el 2026-07-17 con la salvedad del propio artículo, y el párrafo de H6 de `metodologia.tex` la nombra desde hoy («sobre la velocidad y sobre el gap»); la disparidad no entra al veredicto aunque su artículo afirme correlación positiva con el error de test, porque esa predicción nunca se registró y añadirla con el signo ya visto sería elegir la predicción después del dato, así que se reporta como observación y se dice que sale al revés; lo mismo para la stiffness con el gap, que va en el sentido del título de su artículo. Sin figura, porque son recuentos.

**Por qué.** H6 no calcula nada nuevo: son las cifras de la tabla de H1 partidas por el signo que cada artículo afirma. La tabla de predicciones es la tabla de signos de [[Datos experimentales]] §5.3 por variable, con GWA convertida y la m-coherencia sobre la escala de ruido con el signo cambiado, y es la misma tabla que la lectura de selección usa como extremo bueno.
...
**Qué se decidió.** El orden de H3 se calcula con la regla del protocolo, celdas con asociación del signo mayoritario y desempate por la mediana de |D| (`results/ranking.parquet`), con tres concreciones fijadas antes de calcular: el signo mayoritario es el que tiene la D en más celdas y un empate lo decide la mediana; la posición en la rejilla va como fila de referencia en el puesto que le tocaría, sin número; y la segunda mitad de H3, «el orden cambia de bando», se lee repitiendo la cuenta dentro de cada conjunto de datos y de cada arquitectura (`ranking_grupos.parquet`) y mirando solo la primera métrica de cada grupo. La poda se cuenta como resultado propio y sus tres parejas se vuelven a medir con el estadístico del contraste, la D dentro de celda al 5 % sobre los 806, en vez de copiar las Spearman del 2026-09-03.
...
**Veredicto.** H4 se sostiene para las métricas: su |D| no crece con la ventana. La ventana mínima defendible es la primera, y no porque maduren pronto sino porque no maduran; la referencia gratuita sí madura hasta 0,70. Escrito en `resultados.tex` §Suficiencia de la ventana temprana.
...
- **Trampa, superada el 2026-09-05:** la figura `ventanas` llevaba diez líneas por panel con la mediana de |D| en cada ventana y no se entendía (cuatro puntos no equiespaciados unidos con líneas, y las medianas ya están en la tabla). Pasó a un diagrama de caja por predictor del cambio de |D| por celda, leído de `results/ventanas.parquet`, con la misma afirmación, las gratuitas suben y las métricas se quedan planas. El mismo día `signos` pasó de 240 puntos por panel a cajas por familia, y la significación por celda vive solo en la tabla de recuentos.

#### La fase C fija tres reglas de lectura de H2 antes de calcular su tabla, y H2 cae

**Qué se decidió.** Superar a la referencia se lee con el intervalo de la diferencia |D| de la métrica menos |D| de la referencia, por jackknife pareado sobre los mismos entrenamientos (columnas `D_diff`, `se_diff` y sus `_land` de la tabla larga), con el recuento crudo de celdas al lado como descripción; redundante es |D_ref| ≥ 0,8, el mismo umbral que la poda; la pérdida por selección se lee por mediana y por celda, con el signo del artículo como primaria y el signo invertido solo como exploratorio (`regret_flipped` en `seleccion.parquet`). Fijadas con H1 leída y la tabla de H2 sin calcular; las medianas de pérdida se habían visto de reojo el 2026-09-03 y así se declara.
...
**Lo que se deja como estaba, y por qué.** No se niegan las columnas de GWA para recuperar la convención del artículo: la convención actual está escrita de forma coherente en seis sitios y cambiarla ahora abre más errores de los que cierra; se escribe en Fundamentos su forma cerrada como margen normalizado. No se añaden variables dependientes nuevas, porque la multiplicidad ya es el problema. No se cambia la columna titular de la *stiffness*: el coseno global a normas iguales es (α − 1)/(M − 1) y sería redundante con la escala de ruido. No se comprime Resultados ahora: se retiran solo las figuras `solape-bandas` y `solape-cruces` y la compresión queda para la fase H (esa misma noche se retiró también `val-test`, se borró `solape-mapa` del todo, porque no se entendía en ninguna de sus dos versiones, y se recortaron §Concordancia, §Rango dinámico, §Riesgos, §Protocolo, §Calibración, §Cálculo de las métricas y el anexo de configuración, de 101 a 96 páginas). No se reabre el micro-benchmark de coste (decisión del 2026-08-27); se añade el sobrecoste medido por celda.

**Cronología, para declararla en la memoria.** Método fijado por la mañana. Tabla larga, figura `signos-h1` y lectura de H1 en `resultados.tex` §Existencia de la asociación (recuento de signos con Benjamini-Hochberg) por la tarde, del flujo de análisis. Revisión externa hecha sin calcular ninguna correlación métrica-VD; sus motivos son los de arriba, todos de estructura. Las diez desviaciones se decidieron después de conocer esa primera lectura, y así se declara en Metodología §Riesgos, con Willroth y Atherton 2024 como forma de reportarlas. Ninguna se justifica por el valor de un coeficiente.

**Evidencia medida sobre `reports/`.** Suavizado: los 12 umbrales de la norma del 60 % no cambian; los 806 que aprendieron siguen siendo 806 pero cambia uno, sale `fc_cifar100_sgd_lr0.3_seed2` (cociente 1,14) y entra `fc_cifar100_adam_lr0.01_seed4` (1,32), los dos colapsados, y 1,2 y 1,25 ya no seleccionan el mismo conjunto (153 frente a 154); los cruces pasan de 571 a 573 y 17 runs cambian de *epoch*; los cruces en la *epoch* 1 bajan de 17 a 2; `best_val_acc` cambia en 270 runs y `best_val_loss` en 213 (mediana del cambio relativo 0,2 %, máximo 29 %; en 64 cambia la *epoch* del mínimo); la fracción de cruces consumida al cerrar el 5 % baja del 10,7 al 10,3 % y la del 10 % queda en el 34,6 %. Poda: NGV contra escala de ruido, Spearman 0,94 dentro de celda al 5 % (mínimo 0,79) y −0,94 contra la m-coherencia; disparidad contra `noise_scale/tr_sigma`, Spearman 0,98, cociente medido 0,038 frente a 0,039 teórico. GWA contra el *accuracy* de validación al 5 %: −0,83. Valores de referencia de los estimadores (corregido el 2026-09-06, aquí decía «techos» y «satura»): la escala de ruido tiende a M − 1 = 255, con mediana 207 y lecturas por encima; NGV tiende a K = 10, con mediana 8,1.

- **Trampa (corregida el 2026-09-04, en la revisión de las fases C a G):** el *jackknife* supone runs independientes, y los 40 de una celda no lo son, porque los ocho *learning rates* de una *seed* comparten inicialización, orden y probe; el intervalo por run se mantiene y se declara ancho. Dice cuánto se movería la D con otras *seeds* sobre la misma rejilla y nada sobre otra rejilla, otro conjunto ni otra arquitectura.
...
**Superada esa misma noche.** `solape-cruces` y `solape-bandas` salieron de la memoria con las diez desviaciones, y `solape-mapa` se borró del todo, función, prueba y PDF, porque no se entendía ni como mapa de calor ni como un punto por celda sobre el eje. Los recuentos por celda y ventana viven en la prosa de §Solape, y la única figura de esa sección es `solape-celda`.

#### La columna titular de cada predictor pasa a la memoria, tal como estaba marcada en el código

**Qué se decidió.** Las once columnas titulares marcadas en el código (las ocho métricas, TSE con `tse/ema_0_999`, y `val_loss` y `val_acc`) se escriben en `metodologia.tex` §Variables como la Tabla «Columna titular de cada predictor», y Resultados §Rango dinámico las cita por esa tabla. Sin cambiar ninguna: son las que usó el punto 3 de la fase A y las que dibuja la figura de rango por columnas.
...
**Qué se decidió.** Solo coexisten al final, así que ahí se comparan `final_val_acc` con `final_test_acc` y el `val_loss` de la última *epoch* con `final_test_loss`. Orden: tau-b de Kendall por celda. Nivel: validación menos test por run contra la desviación típica binomial de dos *accuracies* sobre los tamaños de las particiones, dos desviaciones como margen. Fuera los 41 divergidos. Dos pasadas. Cierra de paso la nota de honestidad F1 macro ≈ *accuracy* de la fase H. Método, evidencia y lecturas en `resultados.tex` §Concordancia entre validación y test.
...
- **Una figura y no dos, y desde el 2026-09-03 ninguna.** `val-test` se retiró de la memoria esa noche porque comprueba algo que el diseño garantiza; la sección quedó en tres párrafos con sus números, y la función que la genera sigue disponible en el código.

#### El punto 4 mide el solape en pares, sobre las tres variables de velocidad, y fija la mitad como regla

**Qué se decidió.** Una ventana se cierra en la *epoch* en la que se lee el predictor y un suceso en esa misma *epoch* cuenta como ya ocurrido. Suceso de VD1, el cruce; de VD3, la *epoch* del mínimo del *loss* suavizado; VD2 no tiene suceso y se mide la parte del área que la ventana aún no fija. Lo que sigue por delante se cuenta en pares comparables, con la fórmula de la censura del 2026-08-31. **Regla:** una ventana sirve para una variable en una celda cuando al menos la mitad sigue por delante. Dos pasadas. La ventana se lee de `metrics_at_window.parquet`. Método, evidencia y lecturas en `resultados.tex` §Solape entre la ventana y el desenlace.
...
- **Cuatro figuras:** `solape-celda`, `solape-cruces`, `solape-bandas` y `solape-mapa`. Solo `solape-celda` queda en la memoria desde el 2026-09-03; ver la entrada de esa fecha.

#### El estilo de figuras pasa a color de artículo, y se retiran tres restricciones

**Qué se decidió.** Salen del estilo de figuras la separación mínima de luminancia, el emparejamiento color-trazo y la tipografía del cuerpo dentro de las figuras. Paleta **Okabe-Ito** y tipografía **TeX Gyre Heros**. Se mantiene lo que no es estilo: ancho final sin escalado, prohibición de `bbox_inches="tight"`, salida en PDF, eje hasta el cero y escala compartida entre paneles.

**Por qué.** El depósito por Ebrón es digital y nunca habrá entrega en papel monocromo, así que el color puede informar solo.
...
- **Regla:** el color sigue al concepto y nunca al rango, y la paleta se asigna en su orden declarado.

#### El punto 3 mide el rango dinámico con eta cuadrado sobre puestos, y no entra ninguna biblioteca nueva

**Qué se decidió.** Dentro de cada celda, columna y ventana temprana, los valores se sustituyen por su puesto y la dispersión se parte entre grupos y dentro de grupos, una vez por learning rate y otra por semilla, contra la referencia exacta (k−1)/(n−1). Dos pasadas. Método, evidencia y lecturas en `resultados.tex` §Rango dinámico del predictor.

**Por qué un tamaño de efecto y no una prueba.** La pregunta es cuánto sitio tiene el predictor para moverse; un valor p serían 2.592 contrastes sin que ningún objetivo los pida. El cociente es el ε² de Kruskal-Wallis, así que una biblioteca de estadística solo aportaría ese valor p.
...
- **Cambio de plan, con fecha.** La fase A decía que solo el punto 2 llevaba figura; el punto 3 se lleva dos, `rango-celda` y `rango-columnas`.
...
- **Trampa: la figura agrega los cinco runs de cada posición** y no dice cuál cruza; el detalle run a run está en el mapa de lo computable. La trampa del eje x no compartido dejó de existir el 2026-09-02, cuando los dos paneles pasaron a un eje común.
...
**Qué se decidió.** Por cada run y VD, tres estados: el valor está, falta, o está pero no es una medición. El tercero existe solo en `final_test_acc` y `final_gap_acc` de los 41 divergidos, donde `argmax` sobre logits NaN devuelve el índice 0. El mapa señala y no excluye. La censura de VD1 se cuenta en pares comparables: un censurado se ordena contra cualquier run que cruzó, y con 15 cruces de 40 quedan el 61,5 % de los pares y no el 37,5 % que sugiere contar runs. La memoria lo cuenta en §Qué variables quedan medidas.
...
**`reports/` se parchea:** son la fuente de la verdad y se corrigen en vez de enmascararse al leer. La regla exige coseno del mismo ámbito a NaN y celda a 0,0 exacto, porque un subconjunto de pares vacío da un 0,0 legítimo. 41 runs, 82 ficheros, 6.484 celdas de 0,0 a NaN; no se re-entrena nada.

- **Aguas abajo no hay nada que hacer:** desde este día `reports/` trae el NaN y se comporta como cualquier valor ausente.
- **En la memoria no se narra.** La memoria describe un método y reporta resultados, y ningún resultado dependió del fallo, porque se encontró antes de la primera correlación. La única obligación es no afirmar lo contrario: no escribir que `reports/` es exactamente lo que produjeron las ejecuciones.
...
- **El NaN de GWA no es un defecto:** GWA devuelve NaN a propósito cuando la varianza de los cosenos es cero.

#### El `README.md` certificaba lo contrario de la verdad; el pilot pasa a versionado y `CLAUDE.md` no se versiona nunca

El `README.md` afirmaba que el historial de git certificaba que el plan precedía a los datos; corregido con la cronología, la matriz terminó el 22 y el plan se retiró el 25. `reports_pilot/` pasa a versionado, 2,5 MB con los seis `testfix_40ep/` de Tiny y los `summary.json` que se auto-declaran corruptos con `_tiny_test_note`, porque es la evidencia de `DATASET_BUDGET`. `CLAUDE.md` no se versiona nunca, porque es un fichero de instrucciones para una IA y el repositorio acaba depositado; queda en `.gitignore` con el motivo al lado.

#### La fase A cuenta pero no excluye, y su veredicto se parte en dos en la memoria

La fase A cuenta, no excluye: un run clavado en el azar tiene *accuracy* de test y entra en el recuento, y descartarlo es método y es fase B. La correlación entre dos VD, el punto 5, está permitida porque no entra ninguna métrica. El veredicto va partido: los números a Resultados, porque son algo calculado sobre la matriz, y la regla que se derive, la población de análisis, a §Protocolo de análisis, que escribe la fase B.
...
**El coste por métrica no existe en los 960 entrenamientos**, y no es que no se midiera: cada `summary.json` guarda un único `metric_seconds`, y `stream_shared` ejecuta el barrido por ejemplo una sola vez para seis de las ocho métricas, así que su coste marginal individual no es recuperable ni a posteriori. Peor aún, con un barrido compartido **"el coste de la m-coherencia" no está bien definido**: si se calculan las seis, su coste marginal es casi nulo; si se calcula sola, paga el barrido entero. Ese es el eje que el Estado del arte prometía como frente de Pareto.

**Decisión: primero el coste teórico en notación asintótica; el micro-benchmark se aparca.** El coste asintótico es mejor que el reloj para este fin, porque no depende de la máquina ni de qué otras métricas corran a la vez, y separa las ocho en ligas según cuántas pasadas hacia atrás exigen, que es la unidad que manda.

- **Consecuencia que hay que asumir:** con coste asintótico el eje deja de ser continuo y pasa a ser una escala de tres o cuatro niveles, así que lo que se puede dibujar honestamente **no es un frente de Pareto** literal. Resuelto el mismo día en los cuatro sitios de la memoria que lo prometían: lo que se compara es dentro de cada clase de coste y frente al predictor de referencia, que no calcula ningún gradiente, en Resultados §Coste y capacidad predictiva. La sección se mantiene en lugar de doblarse dentro de OE2, porque las clases de coste dan material propio y OE2 pregunta otra cosa; esa parte es revisable.
- **La tabla del estado del arte no se toca y el coste va en una tabla nueva** en Desarrollo §Cálculo de las métricas, con un solo propósito, lo que cuesta medir y lo que ahorra el barrido compartido. La memoria se queda fuera de esa tabla y sigue contada en prosa, porque responde a otra pregunta.
...
Definido desde cero en un módulo propio, con las notas de [[Estilo de redacción - notas del TFM HOFT]] §Estilo de las figuras como referencia, y aprobado sobre una demo con datos inventados. Aquí quedan las medidas y las trampas.

Medidas antes de decidir, no supuestas: el bloque de texto son **15 cm** (A4 con 3 cm de margen por lado), el cuerpo son **10 pt en Palatino**, y **TeX Gyre Pagella**, el Palatino libre, está en el árbol de TeX Live.

- **La figura se genera al ancho final y LaTeX no la escala nunca.** Solo hay dos anchos, 15 y 10 cm, y los fija el módulo, así que en el `.tex` no debe aparecer ningún `width=0,8\textwidth`. Ataca el defecto medido en HOFT, donde dos gráficas quedaron con los rótulos a la mitad del cuerpo por exportarlas a un tamaño y encogerlas a otro.
- **Trampa que anula lo anterior si se toca:** recortar el PDF al contenido al exportar, el ajuste «tight» habitual de matplotlib, **cambia el tamaño final**. Es justo lo que alguien «arregla» sin saber que lo rompe.
- **Reglas fijadas al aprobarlo.** Salida **siempre en PDF**, porque un PNG es previsualización y nunca entregable. **Ningún eje cortado por encima del cero**. Y **dos paneles de la misma cantidad comparten escala**. Las dos últimas están en el código con prueba, porque la demo aprobada las incumplía las dos.
- **Separación deliberada de HOFT:** fuera los ejes superior y derecho; allí la caja cerrada es el valor por defecto de matplotlib y no una decisión.
- **Se copia la regla de figura o tabla de HOFT:** cómo cambia algo al mover una perilla continua va a gráfica, qué gana en qué prueba va a tabla con el mejor en negrita, y cómo funciona un mecanismo va a esquema. Junto con la regla de una figura, una afirmación.
- La regla del color emparejado con el trazo se retiró el 2026-09-02; ver la entrada de esa fecha.
...
Se borran `4 - Análisis.md` (el plan preregistrado y congelado), el módulo de análisis de potencia con sus pruebas, y el §Protocolo de análisis de [[1 - Diseño]]. Las seis hipótesis se conservan en [[1 - Diseño]] como afirmaciones falsables, ahora sin criterio de decisión.
...
Verificado sobre los 960 runs. Los techos de val-accuracy de la FC son 0,584 y 0,569 en CIFAR-10, 0,295 y 0,289 en CIFAR-100 y 0,114 y 0,108 en Tiny-ImageNet, con ninguna de las ocho tasas de aprendizaje ni ninguna de las cinco semillas. Con el umbral único por conjunto de datos de entonces (0,65 / 0,35 / 0,20), esas seis celdas no cruzaban nunca, y por eso el umbral pasó a depender también de la arquitectura el 2026-09-01. No es falta de presupuesto ni un fallo de configuración: la misma red sí cruza en MNIST, 0,987 y 0,985 contra 0,97, y el pilot la corrió al presupuesto doblado, 80 épocas, con techo en la 40. Es el techo de la arquitectura.

#### Muchos entrenamientos se quedan clavados en el azar, y se detectan por un cero exacto

Son **154 de 960**, repartidos por 16 de las 24 celdas, así que no es una curiosidad de un conjunto concreto. **El mecanismo, comprobado:** las ReLU mueren, la última capa recibe un vector de entrada exactamente nulo y el clasificador solo emite su sesgo; el gradiente del peso de la última capa es entonces exactamente cero y, como GWA protege la norma con un mínimo, `gwa/score_mean` sale **0,0 exacto**. Un cero exacto en coma flotante no sale por casualidad, así que sirve de firma: aparece en 124 runs y ocupa de media el 74 % de sus épocas. No es infalible, porque 9 de esos 124 no acaban clavados.

Qué se hace con ellos al analizar se decidió el 2026-09-03: fuera de la población primaria de todo contraste. En la variable de velocidad no había nada que decidir, porque un run clavado nunca cruza el umbral.

### 2026-08-05

#### Corregido el estadístico de degeneración de los diagnósticos de sanidad

El diagnóstico de «¿esta métrica llega a moverse dentro de un entrenamiento?» normalizaba contra una referencia calculada **entre** entrenamientos y dominada por el de mayor escala, así que ordenaba por escala y no por movimiento. Lo sustituye la desviación típica de los valores dividida por la desviación típica de sus primeras diferencias, dentro de cada run: numerador y denominador escalan igual, así que no depende de las unidades, y trae su propia referencia, porque un ruido blanco alrededor de una constante vale 1/√2 ≈ 0,71.

El orden se invirtió en la cabeza y en la cola. `var/normalized` y `noise_scale/simple` parecían las más sanas y son las que menos se distinguen del temblor (0,81 y 0,82); `gwa/value` parecía la más muerta sin serlo, porque lo que la normalización anterior confundía con inmovilidad eran seis órdenes de magnitud de escala entre celdas.
...
Cierra el "registrar aquí los valores finales con su evidencia" de la decisión del pilot (2026-06-09). Los valores operan en la configuración central y en los 24 YAML desde el 2026-06-17 (evidencia: el informe del pilot sobre `reports_pilot/`; el pilot corrió con los presupuestos candidatos doblados, 40/80/120/160 épocas). Los umbrales que esta entrada fijó dejaron de valer el 2026-09-01, cuando pasaron a depender también de la arquitectura.
...
Confirmado por el tutor el 2026-06-14 e implementado el mismo día. Añade el constructo *generalización* a las variables dependientes de [[1 - Diseño]], junto a velocidad y rendimiento final.
...
Cada run cronometra por separado el entrenamiento y la instrumentación: `summary.json` gana `total_seconds`, `metric_seconds` (acumulado alrededor de cada bloque de medición, sincronizando la GPU para que el tiempo se atribuya al reloj correcto) y `train_seconds` = total − metric.
...
- **Convenciones.** La evaluación por época cuenta como entrenamiento (práctica estándar de cualquier run); solo la medición de métricas y el baseline TSE van al reloj de overhead. La sincronización se hace solo alrededor de las mediciones (infrecuentes y ya caras), nunca por paso de optimización. El wall-clock es señal de presupuesto y anomalías, no métrica científica, así que no se correlaciona como si fuera limpio.

### 2026-06-09

#### Pilot de calibración: un run por celda, presupuesto doblado

Concreta el "se calibran en el pilot" de presupuestos y umbrales (decisión "Matriz de runs congelada") en un protocolo ejecutable con un lanzador aparte del de producción.
...
- **Aislado de `reports/` a propósito.** Los pilots escriben en `reports_pilot/`: el lanzador de la matriz da por hecho un punto de la rejilla si existe `reports/<run_name>/summary.json`, y un pilot con LR de rejilla y seed 0 dentro de `reports/` se contabilizaría después como run de producción, entrenado con el presupuesto viejo.

#### Justificación valor a valor de los hiperparámetros congelados

La matriz congelada fija números concretos (en la configuración central y en cada YAML de `experiments/`). Aquí queda el porqué de cada uno; los que ya tienen decisión propia (rejilla de LR, seeds, métricas) solo se referencian.

- **`batch_size = 128`.** Tres razones. (1) *Comparabilidad de las métricas*: varias métricas dependen del batch de entrenamiento. La gradient disparity no es comparable entre runs con batch distinto (su varianza decrece como $1/m$; aviso documentado en la nota de Forouzesh & Thiran) y el GNS se lee relativo a B ($\mathcal{B}_{\text{simple}} \approx 25 \cdot \text{NGV}$, exacto en esperanza, con 25 el tamaño de submuestra de la NGV; hasta el 2026-09-06 esta entrada decía «B·NGV por TLC», que era una lectura equivocada de la misma relación). Barrer B mezclaría el predictor con el hiperparámetro; fijarlo lo neutraliza. (2) *Es el valor del corpus*: Sankararaman et al. (gradient confusion) usan exactamente mini-batches de 128 en el mismo trío MNIST/CIFAR-10/CIFAR-100. (3) *Práctico*: cabe en memoria con las cuatro combinaciones dataset×modelo y da longitudes de época razonables (351 pasos/época en CIFAR, 390 en MNIST y 703 en Tiny-ImageNet tras recortar validación; la cifra de junio, 391/469/782, contaba el train oficial entero).
...
- **`probe_size = 256` (M).** Equilibrio entre memoria y estadística. *Memoria*: la matriz de gradientes per-sample pesa ~M×P×4 bytes. *Estadística*: M=256 da 256·255/2 ≈ 32.6k pares para las métricas del Gram per-ejemplo (confusion, stiffness, m-coherence), varianza muestral sobrada para un estimador por medición. *Comparabilidad*: M se congela porque cambia la varianza de todos los estimadores; con probes de distinto tamaño entre runs, las comparaciones entre modelos dejarían de ser válidas. El probe además es fijo durante el run (mismas 256 muestras siempre): la serie temporal mide la evolución del modelo, no el remuestreo.
