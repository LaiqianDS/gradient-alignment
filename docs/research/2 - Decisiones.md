...
**Ninguna pendiente bloquea hoy.** El método de análisis, abierto el 2026-08-25, quedó fijado el 2026-09-03 para los seis objetivos (ver el log de esa fecha), posterior a los datos y anterior a cualquier coeficiente entre una métrica y una variable dependiente. Esa misma tarde, tras la revisión externa de la memoria y después de la primera lectura de H1, se desviaron diez puntos del método, declarados con fecha y motivo en la entrada «Diez desviaciones» del mismo día; ninguno se justifica por el valor de un coeficiente. La forma propuesta el 2026-08-31 se sustituyó en tres puntos, Wilcoxon por el recuento de signos, la parcial de rangos por la cuenta granulada, y el riesgo de la relación en U resuelto sin mirar ninguna nube; el detalle y el porqué están en las entradas del 2026-09-03. Lo que la fase B decida al programar, la tolerancia de la clasificación de formas y el umbral de poda, está ya fijado ahí y solo se revisa con fecha.

Sigue viva la **convención de registro** del censurado que `metodologia.tex` §Variables declara desde el 2026-08-27: un valor censurado se anota como ausente y nunca como el presupuesto. La población de análisis y el tratamiento del censurado en el contraste están decididos desde el 2026-09-03 y se escriben en §Protocolo de análisis con la fase B.
...
- **Trampa:** la lectura de H1 de la tarde y la Tabla de recuentos de `resultados.tex` quedan obsoletas y se reescriben con los intervalos después de regenerar `results/`; los números de la memoria que salgan de ahí llevan la fecha del fichero.
...
**Por qué.** Para un estadístico de rangos, dos métricas que ordenan los runs exactamente al revés llevan la misma información que dos que ordenan igual, y la regla de ayer suponía sin decirlo que la D sería positiva; GSNR es señal partido por ruido y NGV ruido partido por señal, así que ordenan al revés por construcción. La escala de ruido se queda porque es la métrica barata del nivel 1 de [[1 - Diseño]] y la m-coherencia la retadora del nivel 2, y una retadora que es la barata con otro nombre no reta nada; además, si se quedara la m-coherencia, la familia de alineación se apuntaría en H3 un resultado que es de variabilidad. Se descartó Spearman entre las dos columnas por ser un segundo estadístico para un solo uso.
...
**Por qué.** La fase B produce un dato, la tabla larga, y las ramas solo lo leen; lo que no la lee es independiente del tronco. El árbol de git es uno solo y sin ramas, así que el paralelismo se sostiene sobre la propiedad de ficheros y no sobre *merges*.
...
- **Trampa:** el anexo ODS tiene dos entregables, el `.tex` y el `.docx` de la plantilla oficial rellenado para Ebrón (instrucciones leídas el 2026-09-03: anexo al final de la memoria y fichero suelto en el depósito; tabla de los 17 con alto, medio, bajo y no procede, y reflexión de 500 a 1.500 palabras).
...
**Evidencia previa, medida el 2026-09-03 sobre los que aprendieron, que condiciona la lectura.** La *seed* explica de 0,03 a 0,08 de la dispersión de las métricas de alineación al 5 % frente a 0,13 de referencia, y la *stiffness* queda bajo la referencia en las 24 celdas; la escala de ruido da 0,17 con 13 celdas por encima y NGV 0,12 con 12. La cuenta granulada solo tiene con qué trabajar en variabilidad, y esa predicción se registra antes de calcularla. Empates en la velocidad entre cruces: mediana del 13 % de los pares por celda, máximo 44 %, tres celdas con cinco o menos valores distintos.
...
- **Trampa:** los runs ya cruzados dentro de una ventana que sirve se quedan; la fracción consumida está medida por celda (punto 4 de la fase A) y se declara junto al coeficiente.
