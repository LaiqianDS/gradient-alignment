...
**Qué se decidió.** Validación y test solo coexisten al final del entrenamiento, así que ahí se comparan `final_val_acc` con `final_test_acc` y el `val_loss` de la última *epoch* con `final_test_loss`. **Orden:** tau-b de Kendall dentro de cada celda, con `scipy.stats.kendalltau`, que ya era dependencia. **Nivel:** validación menos test por run, contra la desviación típica binomial de dos *accuracies* independientes sobre los tamaños de las dos particiones, `NOISE_SIGMAS = 2`. Los tamaños suben a `config.py` como `VAL_SIZE` y `TEST_SIZE`, y `data.py` los lee de ahí, igual que se hizo con `NUM_CLASSES` el 2026-08-31. Fuera los 41 divergidos. Dos pasadas. Vive en `efficiency.py::val_test_agreement` y `agreement_summary`. De paso cierra la nota de honestidad de F1 macro ≈ *accuracy*, que era de la fase H.

**Por qué esas dos cosas.** El orden es lo que consume el estadístico de la fase B, que ordena runs dentro de una celda. El nivel es lo que hace que un umbral fijado sobre validación signifique lo mismo sobre datos nuevos. Ninguna métrica entra, así que es la correlación entre dos VD que la entrada del 2026-08-29 ya declaró permitida.

**Evidencia medida sobre los 919 con medición.** Tau del *accuracy*, mediana de las seis celdas por conjunto: 0,93 CIFAR-10, 0,93 CIFAR-100, 0,92 Tiny, 0,84 MNIST; la del *loss* entre 0,86 y 0,94; las 24 celdas entre 0,71 y 0,97. Nivel: mediana 0,003 en CIFAR-10 y 0,000 en el resto, el 95 % de los runs entre −0,013 y 0,013, y más de 2σ solo el 0 % a 1,8 % según el conjunto, por debajo del 4,6 % que daría el ruido solo. F1 macro menos *accuracy*: mediana 0,002; entre los 790 runs a más de tres veces el azar, p95 0,017 y máximo 0,048. Solo con los que aprendieron, las medianas de tau bajan como mucho 0,03.

- **Trampa: el acuerdo está comprobado al final y solo al final.** El test se mira una vez. Que ambas particiones vengan de la misma distribución y ninguna intervenga hace improbable una discrepancia intermedia, y así se declara.
- **Trampa que importa para la fase B: en MNIST, VD4 no tiene recorrido entre los sanos.** La tau mínima, 0,71 en MNIST con ResNet-18 y Adam, sale de 40 runs que terminan en un tramo de 0,008 de test, el tamaño del ruido. Solo con los que aprendieron, cinco de las seis celdas de MNIST terminan en menos de 0,05. Cualquier correlación con VD4 en MNIST es en buena parte correlación con ruido de medida.
- **Trampa: F1 ≈ *accuracy* solo por encima del azar.** Un run clavado que predice una sola clase da *accuracy* 0,114 y F1 0,020 en MNIST, diferencia 0,093. Son 77 runs con diferencia mayor de 0,02, casi todos en el azar.
- **Trampa: en *loss* la figura muestra desviaciones y son todas de MNIST.** 85 runs tienen un *loss* de test que difiere del de validación en más del 10 %, todos de MNIST, donde el *loss* sano vale 0,02 a 0,1 y una diferencia de milésimas se ve como un 10 a 30 % en escala logarítmica; y los cuatro FC con Adam clavados en el azar dan *loss* de test de 3,0 a 3,8 frente a 2,3 a 2,5 en validación. En *accuracy* ningún run se separa más de 0,02. No toca el orden ni el nivel del *accuracy*.
- **Trampa: la referencia binomial es generosa.** Supone cada ejemplo como una moneda independiente, y los runs clavados tienen *accuracy* fijada por la estratificación, sin varianza. Por eso el observado queda por debajo del 4,6 %; sirve como cota, no como predicción.
- **Una figura y no dos, decisión mía.** `val-test`, dos paneles, *accuracy* y *loss* en escala logarítmica, un punto por run coloreado por conjunto de datos y la diagonal. El mapa por celda no se dibuja porque las 24 tau tienen una sola estructura, la de las celdas con recorrido al nivel del ruido, y eso lo dice una frase.

#### El punto 4 mide el solape en pares, sobre las tres variables de velocidad, y fija la mitad como regla

**Qué se decidió.** Una ventana se cierra en la *epoch* en la que se lee el predictor, y un suceso en esa misma *epoch* cuenta como ya ocurrido, porque métrica y suceso describen la misma red. Para VD1 el suceso es el cruce y para VD3 la *epoch* del mínimo del *loss* suavizado; lo que sigue por delante se cuenta en **pares comparables**, con la fórmula de la censura del 2026-08-31, y un censurado nunca tiene suceso. VD2 no tiene suceso, así que se mide la parte de la suma ponderada del trapecio que la ventana aún no fija, mediana por celda. **Regla:** una ventana sirve para una variable en una celda cuando al menos la mitad sigue por delante, `efficiency.py::AHEAD_FLOOR`. Se calcula dos veces, con todos y solo con los que aprendieron. Vive en `efficiency.py`, `window_overlap`, `overlap_summary` y `vd1_consumed_pooled`, con la ventana leída de `metrics_at_window.parquet` y no recalculada desde el presupuesto.

**Por qué la mitad, y por qué se dice cuándo se fijó.** Una ventana predice cuando el desenlace está por delante para la mayor parte de la información que consume el estadístico. Tolerancia cero vacía el mapa por un solo run madrugador, y sin regla se incumple lo que `metodologia.tex` §Riesgos promete. La regla se fijó **después** de conocer las cifras agregadas del 2026-09-01 y **antes** del desglose por celda, así que no es ciega y la memoria lo dice.

**Evidencia medida sobre los 960.** Las cifras agrupadas en runs reproducen las del 2026-09-01, 11, 35 y 72 %, y añaden el 85 % al 50 %, luego aquel día ya se contó el empate como consumido aunque no constara. Celdas que conservan al menos la mitad, por ventana: **VD1 22, 12, 0 y 0**; VD3 23, 17, 9 y 2; VD2 24, 24, 24 y 13. Las dos celdas que fallan ya al 5 % son FC con Adam en CIFAR-100 y en Tiny-ImageNet, que cruzan en las dos primeras *epochs*. Al 10 % aguantan las seis de MNIST, las cuatro de CNN y ResNet-18 sobre Tiny y las dos de ResNet-18 con SGD sobre CIFAR, y ninguna FC fuera de MNIST pasa de 0,33. Solo con los que aprendieron, VD1 y VD2 no cambian y VD3 da 23, 16, 9 y 3.

- **Trampa: la cuenta es un suelo.** No haber cruzado no es estar por decidir; un run al 95 % del umbral cuenta como por delante. El solape real es mayor.
- **Trampa: VD2 al 50 % cae sobre la línea por construcción.** La ventana fija del área casi exactamente su fracción del presupuesto, 0,96, 0,90, 0,76 y 0,51 frente a 0,96, 0,91, 0,76 y 0,50 de una curva plana, así que al 50 % la regla no decide. Se declara «descrita a medias», ni válida ni descartada.
- **Trampa: VD3 mide el sobreajuste donde el mínimo llega pronto.** En CIFAR-10 con ResNet-18 y Adam los 40 runs tocan su mínimo antes de la *epoch* 10. Es el diseño sin *augmentation* ni *weight decay*.
- **Trampa: el suavizado mira una *epoch* adelante.** La mediana centrada usa la siguiente, luego VD1 lleva una *epoch* de anticipación dentro. Manda la coherencia con VD1 tal como está definida, no la curva cruda.
- **Trampa: las ventanas de MNIST son las *epochs* 1, 2, 5 y 10 y las demás 2, 4, 10 y 20.** Comparar el solape entre conjuntos de datos mezcla tiempo absoluto y relativo. Dentro de una celda es constante.
- **Lo que no decide:** qué hace la fase B con los runs consumidos dentro de una ventana que sirve. Quitarlos recorta el extremo rápido y sesga; dejarlos mezcla detectar con predecir.
- **Dos figuras, por decisión de Lai.** `solape-celda`, la que él pidió, con las 40 curvas suavizadas, el umbral y las cuatro ventanas de CIFAR-10 con CNN y SGD, la celda que reparte sus 20 cruces cinco a cinco; y `solape-mapa`, tres paneles con la misma escala, un punto por celda coloreado por arquitectura y la línea de la mitad, con el recuento de celdas que la superan bajo cada ventana.
...
- **El color informa solo, y el asunto queda cerrado.** Lai confirmó el 2026-09-02 que no habrá nunca una entrega en papel monocromo, así que la impresión en gris deja de ser un caso que considerar. Ninguna figura futura tiene que reservar una segunda señal para ese supuesto.
...
**El hallazgo, que es lo que justifica la entrada.** El barrido del 2026-08-29 dejó dos criterios de decisión sin retirar. Las decisiones de ejecución del 2026-06-09 decían que toda métrica de gradiente se juzga por su valor incremental "ΔR², no ρ crudo", y la entrada del 2026-08-08 citaba de pasada el margen δ_H2 = 0,15. Los dos nombran un estadístico o un umbral para H2, que es justo lo que se retiró el 2026-08-25, y el ΔR² contradice además la propuesta viva de OE2, la comparación pareada por celda. Se retiran los dos, y con ello el repositorio queda por fin sin ningún criterio escrito de antes.

**Por qué con fecha y no en silencio.** La fase H declara que el análisis es posterior a los datos y anterior a los resultados, y esa declaración se apoya en las fechas de este log. Un criterio que muere pide su línea igual que uno que nace. Sin ella, la afirmación del 2026-08-29 queda escrita como si hubiera sido cierta ese día.

- **Trampa: los dos restos sobrevivieron a un barrido que se dio por completo.** El del 2026-08-29 buscaba criterios enunciados como tales, y estos dos iban dentro de entradas sobre otra cosa, una de hiperparámetros y otra de coste de instrumentación. Un barrido futuro tiene que leer las entradas enteras y no solo sus títulos.
...
- **No se cumplió el criterio de calibración preescrito del pilot**, que pedía que CNN y ResNet-18 con el LR central cruzaran hacia el 30-60 % del presupuesto. Con esta tabla las medianas van del 8 % al 25 %.
...
**Qué tipo de censura es.** Donde el suceso ocurre, es censura de tipo I, administrativa: el presupuesto es fijo, igual para todos los runs del dataset y conocido antes de mirar resultados, que es el caso más benigno y el que la estadística de supervivencia trata de serie.

- **Trampa: la ausencia de VD1 no es un agujero de datos, es un resultado.** Lo que se pierde no es información sobre esas celdas, es una de las tres formas de medir velocidad en ellas; VD2 y VD3 no se censuran nunca.
...
**Por qué dos columnas y no una.** Con una sola etiqueta, `resnet18_cifar100_sgd_lr1.0_seed2` se clasificaba como colapsado: colapsa en 5 de sus 40 épocas, se recupera y acaba a 24,7 veces el azar. Cómo acabó y qué le pasó son dos preguntas y piden dos columnas.
...
**El corte va por unidad de observación.** `analysis.py` trabaja sobre columnas de `trajectory.parquet` y se queda tal cual. El código que mira runs y celdas, que lee `summary.json`, va a un módulo nuevo, `src/efficiency.py`. El nombre continúa vocabulario que ya existe en el código y en la memoria, porque las seis VD las produce `train.py::efficiency_summary` y se llaman indicadores de eficiencia.
...
**Qué pasó.** Al validar las columnas sobre los 960 runs, cuatro de ellas daban un cero exacto en 41 entrenamientos mientras las columnas hermanas, calculadas sobre la misma matriz y en la misma función, daban NaN. En esos 41 el entrenamiento divergió y los gradientes son NaN. Casi toda la aritmética propaga el NaN, pero dos operaciones no: `torch.sign(NaN)` devuelve 0,0 en PyTorch, y `NaN < 0` es falso. Así que `stiffness/sign_global`, `sign_within`, `sign_between` y `confusion/frac_neg` publicaban un cero limpio donde no se había medido nada. Se corrigió con dos líneas, una en `stiffness.py` y otra en `gradient_confusion.py`, con una prueba cada una.

**Por qué importaba arreglarlo.** En los runs sanos `stiffness/sign_within` vale 0,90 de media; en las 1.621 celdas afectadas valía 0,00 exacto, y el resultado final de esos runs es el peor posible. El punto falso caía en la esquina del gráfico, métrica en su mínimo y resultado en el peor, así que una correlación habría leído "poca alineación temprana, peor resultado final", que es **la dirección que predice la teoría**. El fallo no metía ruido, metía señal. Un error que mete ruido hace fallar por prudente y se nota; este hace acertar por accidente y no se nota nunca, porque el resultado sale como se esperaba. Era además **invisible para el propio chequeo de validez**: cero está dentro de rango, no es NaN, no es infinito y no rompe ninguna identidad. Lo delató solo compararlo con la columna hermana. De ahí que el punto 1 tenga que mirar los runs y no solo las columnas.
...
**`reports_pilot/` pasa a versionado.** Son 2,5 MB y 120 ficheros, con los seis `testfix_40ep/` de Tiny dentro y los seis `summary.json` que se auto-declaran corruptos con la clave `_tiny_test_note`. Es la evidencia que justifica los presupuestos de `config.py::DATASET_BUDGET` y hasta hoy vivía en un solo disco. Sale con él `reports_validity/` del `.gitignore`, que era un resto de la primera pasada retirada de la fase A y habría dejado sin versionar en silencio cualquier salida futura con ese nombre.
