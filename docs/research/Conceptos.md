...
El gradiente per-sample $g_i = \nabla_W \ell(x_i, y_i; W)$ es el gradiente de la pérdida calculado para un único ejemplo, antes de cualquier promedio sobre el minibatch. Es el objeto sobre el que se construyen casi todas las métricas de alineación, coherencia y varianza, porque toda la información estadística del minibatch se pierde en el momento en que reduces los $g_i$ a su media.
...
Un ejemplo cuantitativo de cómo opera la sobreparametrización a través de la coherencia. Entrenando una red de dos capas en un problema sintético, la confusion empírica $\eta = -\min_{i\neq j}\cos(g_i, g_j)$ decae aproximadamente como $1/h$ con la anchura $h$ de la capa oculta. Para $h = 10$ los gradientes per-sample chocan frontalmente y SGD necesita learning rate decreciente; para $h = 1000$ están prácticamente desacoplados y SGD converge con paso constante.
...
La firma medible de que feature learning está ocurriendo es que cantidades como GSNR o m-coherence, que en un modelo lineal decrecen monotonamente durante el entrenamiento (porque al ir aprendiendo, el gradiente residual contiene cada vez menos señal), en una red profunda real suben durante una fase considerable antes de saturar. Ese ascenso es justo el feature learning ocurriendo: las capas inferiores están alineando las representaciones internas con la estructura del target, lo que aumenta la coherencia entre gradientes per-sample en las capas superiores. La duración y altura de ese ascenso correlaciona con el [[#Gap de generalización]] final, lo que sugiere que medir el feature learning es uno de los proxies train-time más informativos.
...
Lo delicado es de dónde sale $\delta$, porque es donde se cuela la arbitrariedad si uno se descuida. La respuesta correcta es que salga de la pregunta de investigación y no de una tabla de convenciones. En este TFG, $\delta_{H2} = 0{,}15$ está anclado en el coste: instrumentar el gradiente multiplica por ~3,2 el wall-clock del run en el peor caso medido (fc × cifar100), y una correlación parcial cross-celda por debajo de $0{,}15$ explica menos del 2,3% de la varianza de rangos residual, que a ese precio no le compensa a nadie. El $\delta = 0{,}1$ de H4 responde a otra pregunta distinta y por eso es otro número.
