...
- **Coste de instrumentación (decisión cerrada 2026-07-17: mantener la medición completa):** El peor caso medido es fc × tiny_imagenet, con 2153 s de métricas frente a 1993 s de train (~2.08x el wall-clock de entrenamiento), dentro de la cota <3-4x; la prioridad es la serie temporal completa por época. Registrada en [[2 - Decisiones]]; al congelar el plan, incorporarla a su texto.
...
## Pasos inmediatos (rev. 2026-07-17)

**Marcapasos del calendario:** La cadena que queda es congelar el plan → lanzar la matriz (~960 runs, el tramo más largo de septiembre, ahora en una única GPU) → análisis. Lo que bloquea es la congelación, en espera deliberada; de sus tareas previas solo falta fijar el suelo de ajuste del gap.
...
- [ ] **Cobertura de columnas del plan (barato, antes de lanzar):** recorrer §Contrastes del plan y comprobar contra el esquema de un run del pilot que toda cantidad invocada existe ya como columna en `summary.json` / `metrics_at_window.parquet` / `trajectory.parquet`. Es la única carencia que obligaría a re-correr la matriz; el dry-run completo del pipeline sobre datos sintéticos con efecto plantado (Fase 1.1) puede ejecutarse mientras la matriz corre.
...
- [ ] **Congelar (en espera deliberada, 2026-07-17):** antes de mover [[Plan de análisis congelado]] a `docs/research/` faltan el suelo de ajuste del gap (pendiente a propósito, decisión 2026-07-17: es un filtro de análisis y fijarlo después no obliga a re-correr runs) y la incorporación de la decisión de instrumentación al texto del plan; el acto formal se pospone de momento. A partir de la congelación no se mira ningún resultado de matriz sin plan congelado.

**Logística (tuya, en paralelo):**

- [ ] **Smoke test en la GPU única (no hay cluster):** 1 run real de principio a fin si no se ha hecho, antes de la matriz. Hacerlo **por la celda peor y no por la primera de la enumeración**: `--dataset tiny_imagenet --model fc --limit 1` (monta una Gram de ~14 GB en RAM de host y tiene historia de OOM) y `--model resnet18 --limit 1` (la más lenta), relanzando después para comprobar que el launcher las da por hechas. `enumerate_runs` recorre los datasets en orden y deja Tiny al final, que es el 64% del coste: un fallo descubierto ahí llega el quinto día.
- [ ] **Fijar el estado del código antes de lanzar:** commitear lo pendiente (`src/run_pilot.py` con la corrección de `--report`, los `.tex`, el vault, `notebooks/`), etiquetar el commit que produce la matriz y sincronizarlo con la máquina de ejecución. El pilot no corrió en el portátil sino en la caja Linux (los `config.yaml` de `reports_pilot/` llevan `out_dir: /home/lai/...`), así que hay dos copias del repo que pueden divergir; y como `summary.json` no guarda el SHA de git, esa etiqueta es hoy el único vínculo entre los ~960 runs y el código que los produjo.
- [ ] **`reports/` se sincroniza por git** (decisión 2026-07-25; hace además de backup incremental de la Fase 4). Consecuencia a respetar: el commit del plan congelado debe preceder al primer commit de resultados, para que el propio historial certifique que el plan precede a los datos.
- [ ] **Lanzar bajo `tmux`/`nohup` y con la salida a un log:** `execute` corre los 960 subprocesos secuencialmente dentro de un único proceso, así que una desconexión mata la tanda, y hoy el diagnóstico de un fallo solo vive en stdout. La reanudación por `summary.json` cubre el relanzado, no el diagnóstico.

**Tras congelar:**

- [ ] **Lanzar la matriz** por tandas una vez congelado el plan (con una única GPU el troceado por dataset/nodo no aplica); el launcher reanuda los pendientes por `summary.json`. El detalle posterior (QA continuo, backup incremental, ejecución del pipeline de análisis y figuras) está en el plan por fases de arriba (Fase 4 y Fase 5).
- [ ] **Escribir el pipeline confirmatorio mientras la matriz corre** (Fase 1.1 + su dry-run con efecto plantado). Se desarrolla sobre datos sintéticos, no sobre `reports/`, así que no roza la regla de no mirar resultados, y convierte los ~6 días de GPU en tiempo útil en vez de espera.

**No bloqueante, desde ya:**

- [ ] **Redacción en paralelo** (no depende de resultados): completar metodología y dejar listo lo que no espera datos (implementación); intro, estado del arte y fundamentos ya redactados.
