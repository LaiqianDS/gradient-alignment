...
- **Código.** `src/analysis.py` reúne los diagnósticos de sanidad: validez, identidades, columnas ausentes, degeneración, tendencia y redundancia. Desde el 2026-08-29 apunta por defecto a `reports/` y no al piloto. Desde el 2026-08-31 tiene un hermano, `src/efficiency.py`, cuya unidad es el run y la celda: ahí viven el censo de salud y el mapa de lo computable, y ahí irán los puntos 4 y 5. **No hay ninguna figura ni una línea de análisis confirmatorio.** El estilo de figuras sí existe, en `src/figstyle.py`, y la primera figura real llega con la fase A. Suite en **244** pruebas verdes.
...
- [ ] Mapa de lo computable celda a celda. **Contado el 2026-08-31**, falta la figura. `efficiency.py::vd_status`, `availability_by_cell` y `vd1_information` sobre los 960. Resultado: las cinco VD que no dependen de un umbral están disponibles en las **24 celdas** con 30 runs o más (919 a 921 de 960, mínimo de celda 30); VD1 lo está en **18 celdas** y con mediana de 20 runs (494 de 960). Las 41 pérdidas de las cinco son todas divergencias de SGD. La afirmación de la figura sale de ahí: lo que cuesta caro no es la censura, es depender de un umbral.
