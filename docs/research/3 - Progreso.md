...
- **Código.** `src/analysis.py` reúne los diagnósticos de sanidad: validez, identidades, degeneración, tendencia y redundancia. Están escritos como diagnóstico de consola sobre el piloto, no como veredicto sobre los 960. **No hay ninguna figura ni una línea de análisis confirmatorio.** El estilo de figuras sí existe, en `src/figstyle.py`, y la primera figura real llega con la fase A. Suite en **227** pruebas verdes.
...
- [ ] **Decidir el método de los cinco puntos y registrarlo en [[2 - Decisiones]] antes de programar nada**, incluido dónde vive el veredicto en la memoria: hoy no hay sección para él, porque §Protocolo de análisis es de la fase B y §Riesgos ya está escrita en cualitativo.
...
- [ ] **Copia de seguridad de `reports_pilot/`.** Está en `.gitignore` y solo existe en el disco local: si se pierde, se pierde la evidencia que justifica los presupuestos y umbrales congelados. Es además requisito previo si alguna vez se quieren meter en la memoria las cifras del régimen post-meseta.
