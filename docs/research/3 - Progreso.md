...
- **Código.** `src/analysis.py` reúne los diagnósticos de sanidad: validez, identidades, columnas ausentes, degeneración, tendencia y redundancia. Desde el 2026-08-29 apunta por defecto a `reports/` y no al piloto. Desde el 2026-08-31 tiene un hermano, `src/efficiency.py`, cuya unidad es el run y la celda: ahí vive el censo de salud y ahí irán los puntos 2, 4 y 5. **No hay ninguna figura ni una línea de análisis confirmatorio.** El estilo de figuras sí existe, en `src/figstyle.py`, y la primera figura real llega con la fase A. Suite en **239** pruebas verdes.
