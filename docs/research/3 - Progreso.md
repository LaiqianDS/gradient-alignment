...
- **Código.** `src/analysis.py` reúne los diagnósticos de sanidad de las columnas y apunta por defecto a `reports/`. `src/efficiency.py` tiene por unidad el run y la celda, ahí viven el censo de salud y el mapa de lo computable, y ahí irán los puntos 4 y 5. Las figuras viven en `src/figures.py`, una función por figura, y ahí irán también las de las fases B a G. **Hay tres figuras** (`ventana-lr` del punto 2, `rango-celda` y `rango-columnas` del punto 3), todas referenciadas desde Resultados, y **sigue sin haber una línea de análisis confirmatorio**. El reparto del punto 3 lo calculan `analysis.py::dynamic_range_report` y `dynamic_range_summary`, sin biblioteca nueva. `figstyle.py` registra las cuatro variantes de Pagella y no solo la redonda, así que un rótulo en cursiva sale en la cursiva del cuerpo; el modo matemático de matplotlib se probó y se descartó, porque imprime un aviso de tipografía en cada figura. Desde el 2026-09-02 la paleta es monocroma, cuatro tintas separadas en luminancia, por decisión de estilo. Suite en **258** pruebas verdes.
...
- **Memoria.** Escritos enteros Introducción, Estado del arte, Fundamentos, Desarrollo y Trabajos futuros. De Metodología **solo queda vacía §Protocolo de análisis**, que escribe la fase B. De Resultados están escritas §Qué variables quedan medidas y §Rango dinámico del predictor, que abren el capítulo por el lado de las variables a predecir y por el del predictor; las otras ocho secciones y Conclusiones siguen siendo títulos sin prosa, y la hoja de ruta y las conclusiones del capítulo se escriben con el cuerpo. §Hipótesis da las seis afirmaciones y lo que refutaría a cada una, sin criterio numérico y sin ninguna palabra que sugiera preinscripción, porque el método es posterior a los datos. **Tres figuras y una ecuación propia**; sigue faltando la única del capítulo de Desarrollo, el diagrama del *pipeline*, anotada como comentario en su fichero. Faltan los tres resúmenes de `main.tex`, los agradecimientos y los dos anexos. Compila en 63 páginas, sin referencias sin resolver ni cajas desbordadas.
...
- **La correlación de la fase B tendrá una causa común, medida el 2026-09-02.** Dentro de una celda el learning rate mueve casi toda la métrica, de 0,45 a 0,92 según la columna, y mueve también el indicador de eficiencia. El contraste medirá en buena parte dos efectos del mismo eje. Declarado en `resultados.tex` §Rango dinámico del predictor, y hay que sostenerlo al interpretar cada objetivo.
- **El predictor de referencia tiene más rango dinámico que cualquier métrica cara.** TSE marca 0,97 y las dos lecturas de validación 0,95, contra 0,92 la mejor instrumentada. Empeora el pronóstico de H2 antes de calcular ninguna correlación.
...
**Regla de nombres en las figuras, del 2026-09-02:** dentro de una figura no entra ningún identificador del código. Se escribe el nombre que usa la memoria, con su misma convención de redonda para el acrónimo y cursiva para el término inglés, y sin abreviaturas que no estén definidas en el texto. El mapa de columna a nombre vive en `figures.py::COLUMN_LABELS`, con una prueba que exige una entrada por columna titular. Una figura se lee sola; si un término inglés necesita contexto, ese contexto va en la prosa que la rodea.
...
- [x] **Rango dinámico del predictor. Cerrado el 2026-09-02, con el método y las trampas en [[2 - Decisiones]].** Eta cuadrado sobre puestos, con el learning rate como factor y la semilla como segundo reparto, en `analysis.py`. Las ocho métricas de gradiente superan su línea de referencia, de 0,45 a 0,92, y solo 29 de las 768 casillas caen en ella o por debajo, todas de variabilidad o de m-coherencia. La causa dos de un coeficiente nulo queda descartada. Salieron dos hallazgos que ahora viven en §Vigentes, el rango del predictor de referencia y la causa común.
