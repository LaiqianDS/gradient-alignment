...
- **El umbral de VD1 está en consulta con el tutor** (abierto el 2026-08-31, [[2 - Decisiones]] §Pendientes). Todo lo que dependa de VD1 se calcula con 0,97 / 0,65 / 0,35 / 0,20 y se recalcula si cambia. Recalcularlo es barato: sale de la curva de val de `trajectory.parquet`, sin reentrenar.
...
- **Solape entre la ventana de medida y el cruce del umbral.** Los cruces de CNN y ResNet a LR central caen al 5-15 % del presupuesto, así que en la ventana del 25 % el suceso ya ha ocurrido para buena parte de los runs. Sin cuantificar todavía; es el punto 4 de la fase A y condiciona OE4.
...
- **154 runs nunca aprendieron y los 154 tienen causa conocida.** Que entren o no en cada contraste está **sin decidir** y es de la fase B. La fase A los cuenta, no los excluye.
