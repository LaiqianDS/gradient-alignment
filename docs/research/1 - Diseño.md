...
> **Nota de revisión (2026-08-01, antes de existir ningún dato de matriz).** Los criterios de H1, H3 y H4 que aparecen abajo son la redacción original de junio y quedan **superseded** por [[Plan de análisis congelado]], que es el documento operativo. Se conservan aquí como registro de lo que se planteó, no como lo que se contrasta. Los motivos, por si se citan en la memoria:
>
> - **H3 es el cambio importante.** Su criterio ("la métrica de mayor ΔR² pertenece a la misma familia en ≥ 16/24 celdas") está sesgado de forma no reparable: alineación tiene 5 métricas y variabilidad 3, así que bajo la nula de que las 8 son intercambiables el argmax cae en alineación con probabilidad 5/8, o sea **15 celdas de 24 esperadas por azar**, y la probabilidad de superar el umbral de 16 sin que exista efecto de familia es **0,42**. Cualquier criterio construido sobre un argmax hereda el sesgo del tamaño de los grupos. El plan lo sustituye por la diferencia pareada de medianas de |ρ| por familia dentro de cada celda, insesgada respecto al tamaño de las familias.
> - **H4** decía "ρ al 10% no difiere significativamente de ρ al 50%", que es afirmar la nula: con ICs anchos se "confirma" casi gratis. El plan usa no-inferioridad (TOST) con margen preespecificado. El test de Steiger/Williams que se nombra abajo asume normalidad bivariante, insostenible sobre rangos con empates por censura.
> - **H1** pedía correlación "en la mayoría de condiciones"; el plan contrasta la mediana cross-celda, que no es la misma afirmación (una mediana de 0,3 admite media rejilla a 0,05). El plan reporta además la fracción de celdas con signo consistente para que el lenguaje de "la mayoría" siga siendo honesto.
> - **El ΔR²** se contrastaba abajo con un test F de modelos anidados, que asume errores normales y homocedásticos; sobre rangos con bloques de empates no se sostiene. En el plan es sensibilidad, no contraste primario.
>
> H2, H5 y H6 conservan su formulación, con dos matices que añade el plan: H2 gana un brazo de equivalencia para poder **afirmar** el negativo y no solo no encontrarlo, y H6 gana el criterio de falsación que le faltaba (binomial de concordancia de signos contra 0,5).
...
- **H2 (valor incremental, la decisiva).** Al menos una métrica conserva poder predictivo *tras controlar por el baseline sin gradiente* (TSE-EMA + `val-loss@f` + `val-acc@f`, el titular): su correlación parcial de Spearman es significativa tras FDR (contraste primario), con el ΔR² sobre el baseline (test F de modelos anidados) como análisis de sensibilidad; si discrepan, manda el parcial. (Nota: "ΔR² > 0" a secas no basta como criterio; lo que se contrasta es su significación.) Si H1 se cumple pero H2 falla, las métricas de gradiente son redundantes con la señal que la curva de loss ya da sin coste, un resultado negativo válido y no trivial.
...
- **Valor incremental sobre el baseline.** Más allá del ρ crudo, medir si la geometría del gradiente aporta algo que la curva de loss no contenga ya: correlación parcial de Spearman ρ(métrica, Y | TSE-EMA, `val-loss@f`, `val-acc@f`) como contraste primario, con el ΔR² de añadir la métrica a un modelo que ya tiene el baseline como análisis de sensibilidad (si discrepan, manda el parcial). Preregistrar este análisis junto a las correlaciones crudas. La significación del ΔR² se contrasta con un test F de modelos anidados; la comparación de correlaciones entre ventanas (H4) usa el test de Steiger/Williams para correlaciones dependientes. Ver §Baselines y H2.
