...
- **H2:** la métrica contra su referencia nombrada, val accuracy para VD4 y val loss para el gap, con `D_ref` como medida de redundancia dentro de la celda; y la lectura de selección para VD4, el accuracy de test perdido al elegir el learning rate con la métrica al 5 % frente a elegirlo con la validación al 5 %. Decidida sobre VD4 y VD5, porque en velocidad la referencia es un prefijo de la variable.
- **H3:** ranking de métricas con la familia como etiqueta. **H4:** D al 5 % contra D al 50 % por celda en las variables del final, y en velocidad entre las epochs 1 y 2 (MNIST) o 2 y 4 (resto). **H5:** diferencia pareada SGD menos Adam con intervalo. **H6:** signo predicho a una cola sobre las celdas con intervalo fuera del cero, solo predicciones explícitas.
