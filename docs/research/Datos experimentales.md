...
- MNIST 50k/10k/10k es la convención clásica; CIFAR-10/100 45k/5k/10k sigue a He et al. (2015). En el código solo MNIST comprueba el tamaño exacto del split; las otras tres particiones salen de los tamaños estándar de torchvision (50k train + 10k test en CIFAR; 100k train + 10k val en Tiny) menos el recorte de val, y coinciden con la convención.
...
Más útil que un único número por dataset, porque el estudio cruza tres arquitecturas y el régimen de cada celda depende de ello (la red FC tiene techo por debajo del umbral único antiguo fuera de MNIST, ver [[2 - Decisiones]], y por eso el umbral es por arquitectura). Hay que separar **dos regímenes** que la bibliografía suele mezclar:
...
- **ResNet-18 (stem 3×3).** La variante para imagen pequeña (conv1 3×3 stride 1, sin maxpool) es **exactamente** la nuestra (§2). Sin augmentation, ZeroPur (Liu et al., 2024) reporta la fila «Vanilla»: **83,80 % (CIFAR-10) y 57,01 % (CIFAR-100)**. Ko et al. (2024) dan una columna «No-aug» de 92,4 % en CIFAR-10, pero con weight decay alto (0,4) y 200 épocas, así que es una cota optimista frente a nuestro `weight_decay = 0` y 40 épocas; FractalNet (Larsson et al., 2017) corrobora ~55 % en CIFAR-100 sin augmentation. La banda esperada (wd=0, 40 épocas) queda en ~84–92 % y ~50–57 %.
...
`dataset (4) × arquitectura (3) × optimizador (2) = 24 celdas`. Cada celda barre `8 LR × 5 seeds = 40 runs`, de modo que `24 × 40 = 960` runs. La rejilla de learning rates, las semillas y los hiperparámetros congelados están en [[1 - Diseño]] §Matriz de runs; aquí solo lo que ese documento no da, la definición exacta de cada arquitectura y qué fija cada semilla.
...
Para que el cotejo con la bibliografía sea honesto, así entrena este estudio (valores en la configuración central):

- **Sin data augmentation.** Solo `ToTensor + Normalize` con la media y la desviación del train de cada dataset (estudio sensible al determinismo, [[1 - Diseño]] §Setup de entrenamiento). Son estadísticos de población por canal del split de train, en el rango [0, 1] que produce `ToTensor`:
...
- **Presupuesto de épocas por dataset** (calibrado en el pilot) **y umbral de val-acc por dataset y arquitectura** (fijado sobre la matriz con la norma del 60 %): los valores están en [[1 - Diseño]] §Matriz de runs; los YAML de `experiments/` conservan un umbral viejo por dataset que ningún cálculo lee. El entrenamiento corre **siempre el presupuesto completo de épocas**, y no hay early-stop al cruzar el umbral. El umbral solo se usa **después**, para leer VD1 (épocas hasta cruzarlo) sobre la curva de val-acc suavizada, con el valor crudo en la primera y la última época. Por eso VD2 y VD3 («dentro del presupuesto») están bien definidas, porque la trayectoria completa siempre existe.
...
Los predictores del estudio son las **8 métricas de gradiente** del registro completo más los baselines que no tocan el gradiente. Se miden al final de cada época sobre el probe fijo descrito en §3, y el análisis las lee en las ventanas `f`, la fracción del presupuesto de épocas.
...
Las seis variables dependientes (VD1-VD6), con su definición exacta, la curva sobre la que se lee cada una y el nombre de la clave que emite, viven en [[1 - Diseño]] §Variables dependientes. El `summary.json` de cada run guarda lo que escribió el entrenamiento, pero `epochs_to_threshold` y los tres campos suavizados están calculados con umbrales y un suavizado viejos: la capa de análisis los recalcula siempre desde `trajectory.parquet` ([[3 - Progreso]] §Vigentes).

Aquí no se copian, para que no haya dos versiones que puedan divergir.

`seconds_to_threshold` se mide y se registra, pero conviene no leerlo como indicador de eficiencia, porque el wall-clock de un run mezcla lo rápido que aprende el modelo con la carga que tuviera la máquina y con el coste de la propia instrumentación, que es de algo más de 2x. Las épocas no tienen ese problema.

### 5.1 Salvedad de datos (pilot, Tiny-ImageNet)

Los runs de `reports_pilot/` para Tiny-ImageNet tienen el `final_test_acc` **mal calculado** (bug previo a la corrección), así que VD4 y, por depender del test, VD5 y VD6 no son fiables en esas celdas del pilot; las métricas del lado val (VD1-VD3) y el timing sí lo son. La referencia corregida vive **dentro de cada run del pilot**: `reports_pilot/<run>/testfix_40ep/` contiene la celda re-corrida con la corrección (mismo LR centrado y seed 0, pero al presupuesto calibrado de 40 épocas, no a las 160 del pilot: horizontes distintos, no intercambiables), y el `summary.json` corrupto se auto-declara con la clave `_tiny_test_note`. La referencia sirve para apuntar test-acc/gap por celda, no para análisis (un solo LR y seed). `reports_pilot/` está versionado. El test-acc definitivo de Tiny es el de la matriz completa, corrida ya con el código corregido.

### 5.2 Hipótesis y contrastes

Las seis hipótesis H1-H6 viven en [[1 - Diseño]] §Hipótesis a contrastar, enunciadas como afirmaciones falsables. El método con el que se decide cada una está en [[1 - Diseño]] §Método de análisis, con su base y su cronología en [[2 - Decisiones]].

Las ventanas `f`, la fracción del presupuesto de épocas en la que se leen las métricas, sí están fijadas en la configuración central (`windows = (0.05, 0.10, 0.25, 0.50, 1.0)`) y se registran en `metrics_at_window.parquet`.
...
**Aviso antes de usar la tabla: los signos son los del artículo, y en GWA el nuestro es el contrario.** Hölzl define el gradiente como $-\nabla\ell$ y aquí se mide sobre $\nabla\ell$ bruto, así que el signo observado de GWA se invierte respecto a lo que dice esta tabla y respecto a lo que dice el artículo. La conversión está en Fundamentos §Métricas de alineación y en el enunciado de H6 de [[1 - Diseño]]. Contrastar contra esta tabla sin invertir daría a GWA el veredicto al revés.
...
Verificada contra los PDF (veredictos y citas en [[2 - Decisiones]], entrada «Tabla de signos de H6 verificada contra los papers»). NGV y m-coherence están podadas y sus predicciones se leen sobre la escala de ruido, la de la m-coherence con el signo cambiado; la disparidad cuenta como variabilidad. El reparto real de predicciones fuertes: GSNR la lleva sobre el gap (signo −, vía OSGR; el gap del paper es en loss, la misma cantidad que VD5), y su + vs VD4 es derivación, no claim del paper. GWA la lleva sobre VD4 (signo +; Pearson 0,99 solo en ConvNeXt/CIFAR-10, 0,92 cross-arquitectura, medida sobre el máximo de alineamiento de la trayectoria completa, no en ventana temprana), y su − vs el gap es direccional cualitativo (el paper nunca mide test loss − train loss). m-coherence mantiene el − vs el gap con la salvedad del propio paper (conexión "complicated": con 100% label noise la coherencia también alcanza valores altos; lo informativo según la teoría es la coherencia temprana, lo que favorece la ventana del TFG). La aparente contradicción de signos (+ vs VD4 pero − vs el gap) es solo aritmética, porque VD4 y el gap apuntan a sentidos opuestos de «bueno» (más test-acc es mejor, más gap es peor).

Contrastada en la fase G: la cuenta por predicción está en `results/signos.parquet` y el veredicto en [[2 - Decisiones]]. La predicción de la m-coherencia sobre el gap se contó como fuerte, y la de la disparidad sobre el error de test, que esta tabla registra como base de su extrapolación a la velocidad, entró también como predicción del artículo (signo −, y sale al revés, 12 celdas frente a 4).
...
- Liu, E. et al. (2024). *ZeroPur: Succinct Training-Free Adversarial Purification.* (arXiv:2406.03143). Fila «Vanilla» (sin augmentation) de ResNet-18: CIFAR-10 83,80 %, CIFAR-100 57,01 %.
