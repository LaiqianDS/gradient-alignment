...
$\tau$                & umbral de \emph{accuracy} de validación                  & Sec.~\ref{sec:eficiencia} \\
...
y los pesos del clasificador final, restringido a la última capa lineal~\cite{hoelzl2025gradient}. Para cada ejemplo del \emph{batch} de medición, $\gamma_i = \cos\!\big(g_i^{\text{(últ.)}}, w^{\text{(últ.)}}\big)$, y el agregado por \emph{epoch} corrige la media por el exceso de curtosis de la distribución de los $\gamma_i$:
...
Cada una de estas definiciones lleva implícita una predicción direccional: más coherencia, más \emph{stiffness} intra-clase, menos confusión, menos disparidad o más alineación gradiente-peso deberían acompañar a un entrenamiento más eficiente. Esas predicciones con signo se formalizan como hipótesis falsable en el Capítulo~\ref{ch:metodologia}.
...
\emph{accuracy} de validación en alcanzar un nivel objetivo $\tau$ propio de cada conjunto de datos,
...
Si el \emph{accuracy} no cruza el umbral dentro del presupuesto, el valor queda \emph{censurado}: no se descarta, sino que se trata como el peor resultado posible. Los extremos de la rejilla de \emph{learning rates} no alcanzan el umbral por diseño, de modo que descartar esos entrenamientos sesgaría la muestra. Un $t^\star$ menor indica un entrenamiento más rápido.
