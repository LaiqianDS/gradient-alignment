...
$e$                   & \emph{epoch} en curso, de 1 a $T$                 & Cap.~\ref{ch:metodologia} \\
\bottomrule
\end{tabular}
\end{table}
...
donde $\hat{m}_t$ y $\hat{v}_t$ son las dos medias móviles de Adam corregidas por su sesgo inicial. Con $\mu = 0{,}9$ y un gradiente estable, $m_t$ tiende a $\hat{g}/(1 - \mu) = 10\,\hat{g}$, así que el paso efectivo de SGD es unas 10 veces el de la Ecuación~\ref{eq:sgd} y sigue siendo proporcional al gradiente. El paso de Adam, al dividir cada coordenada por su propia escala, mide del orden de $\eta$ por coordenada sea cual sea la magnitud del gradiente. Esa diferencia es la razón práctica por la que la rejilla de \emph{learning rates} de Adam es la de SGD dividida entre 10 en la Sección~\ref{sec:matriz}. El argumento no es una igualdad de magnitudes, porque los dos pasos dependen del gradiente de manera distinta. Se apoya en los valores por defecto habituales, $10^{-2}$ para SGD con \emph{momentum} y $10^{-3}$ para Adam. La Sección~\ref{sec:matriz} comprueba sobre la matriz que las dos listas dejan en el azar un número parecido de entrenamientos.
...
Los valores registrados muestran esa estructura. Sobre las 33.600 filas de la matriz, el coseno entre pares de la misma clase tiene mediana entre $0{,}30$ y $0{,}51$ en las tres arquitecturas, y el de pares de clases distintas se queda en cero. El coseno medio sobre todos los pares queda en el suelo del ruido en la CNN y en la red \emph{fully connected}. En la ResNet-18 queda unas 30 veces por encima. El argumento de concentración garantiza que una desviación así es un hecho de la red y no una fluctuación del muestreo. Alcanza también a los estadísticos de extremo. Entre los $\binom{M}{2}$ pares del \emph{batch} de medición, el coseno más negativo esperable entre direcciones sin relación tiene magnitud del orden de $\sqrt{2\ln(M^2/2)/P}$, unos $0{,}03$ en la CNN pequeña y $0{,}001$ en las redes grandes. Tampoco un valor apreciable de \emph{gradient confusion} puede atribuirse, por tanto, al azar del muestreo.
...
donde el primer término suma el tamaño al cuadrado del gradiente de cada ejemplo, lo que cada uno aporta por su cuenta, y el segundo suma el producto interno de cada par de gradientes distintos. Ese producto es positivo cuando los dos apuntan al mismo lado y negativo cuando se oponen. En el segundo término vive la coherencia. Las métricas siguientes son distintos agregadores de él.
...
\paragraph{\emph{Stiffness}.} Mide el coseno entre gradientes de pares de ejemplos, desagregado
...
\paragraph{\emph{Gradient confusion}.} Adopta el punto de vista del peor caso y toma el coseno más
...
\paragraph{\emph{Gradient disparity}.} Sustituye el coseno por la distancia entre los gradientes
...
con $\alpha$ la m-coherencia de la Ecuación~\ref{eq:mcoherence}. Ambas dependen solo del cociente $\|S\|^2/Q$, de modo que son la misma cantidad reparametrizada y su correlación de Spearman dentro de un mismo entrenamiento está forzada a $-1$ exacto. El piloto de calibración lo verificó a un error relativo de $8{,}8 \times 10^{-6}$, atribuible al redondeo en coma flotante. La consecuencia es que la escala de ruido y la m-coherencia no aportan evidencia independiente pese a pertenecer a familias distintas, y la NGV tampoco, porque es esta misma razón estimada con 10 submuestras. Las tres son estimadores de $\alpha$, la razón entre $\|S\|^2$ y $Q$; solo GSNR agrega de otra manera, por coordenada. La predicción de signo del artículo de la m-coherencia se lee sobre la escala de ruido con el signo cambiado, porque la Ecuación~\ref{eq:gns-mcoh} es decreciente en $\alpha$.

El estimador de la escala de ruido está cerca de su valor de referencia. Si el gradiente medio verdadero es cero, $\mathbb{E}\|S\|^2 = Q$, $\alpha$ queda en torno a $1$ y la escala de ruido estimada en torno a $M - 1 = 255$; los valores por encima corresponden a $\alpha < 1$, gradientes por ejemplo anticorrelados en media. McCandlish et al. sitúan escalas de $10^{3}$ a $10^{4}$ en CIFAR-10 e ImageNet, muy por encima de lo que 256 ejemplos pueden resolver. Sobre las 33.600 filas de la matriz la mediana es $207$ y el percentil 5 es $58$, y el 12\,\% de las filas queda por encima de $255$. Ese 12\,\% es justo donde el estimador sin sesgo de la señal de McCandlish et al., $\|\bar{g}\|^2 - \operatorname{tr}(\hat{\Sigma})/M$, sale negativo. Ahí la señal no es detectable con 256 ejemplos. Con $\hat{\Sigma}$ la covarianza insesgada, la que divide por $M - 1$, ese estimador vale $(\|S\|^2 - Q)/(M(M-1))$ y es negativo exactamente cuando $\alpha < 1$. Con la traza que divide por $M$, que es la que se registra, la frontera se desplaza de 255 a 256. Las magnitudes registradas no se pueden leer, por tanto, como el tamaño de \emph{batch} crítico que la métrica promete. El orden entre entrenamientos de una misma celda, que es lo que el análisis usa, sí se conserva, porque $M/\alpha - 1$ es monótona en $\alpha$, pero con menos potencia de la que tendría un estimador lejos de esa referencia.
...
agregado por la media sobre los parámetros, nunca por la suma, que sería incomparable entre arquitecturas de distinto tamaño. El estimador usa la varianza insesgada sobre los $M$ ejemplos y descarta los parámetros cuyo gradiente es nulo en todo el \emph{batch} de medición. Bajo señal nula tiene un suelo de $1/M \approx 0{,}004$ por parámetro, porque el cuadrado de la media de $M$ valores de ruido vale en esperanza su varianza dividida por $M$. La media sobre millones de parámetros la domina la cola pesada de $r(\theta_j)$, y por eso se registran también la mediana y el percentil 95. La columna titular es la media por fidelidad a Liu et al. Es esencialmente el recíproco por parámetro de la NGV. Un GSNR alto, que significa que la media del gradiente por ejemplo domina a su varianza, se asocia con un \emph{gap} de generalización pequeño.

Las ocho métricas leen menos cosas de las que su número sugiere. De los $M$ gradientes por ejemplo salen cinco cantidades. La primera es la razón $\alpha$ entre la norma de la suma y la suma de las normas, la Ecuación~\ref{eq:mcoherence}. La leen la m-coherencia, que es $\alpha$, la escala de ruido, que es $M/\alpha - 1$, y la NGV, que es la misma razón estimada con 10 submuestras. La segunda es el coseno entre cada par de gradientes; la \emph{stiffness} toma la media sobre los pares de la misma clase y la \emph{gradient confusion} el mínimo sobre todos los pares. La tercera es la dispersión $\operatorname{tr}(\Sigma)$, que la disparidad mide como distancia entre los gradientes medios de dos submuestras. La cuarta es la razón entre media y varianza coordenada a coordenada, que GSNR promedia sobre los parámetros. La quinta es el coseno entre el gradiente de la última capa y sus pesos, que GWA promedia con la corrección por curtosis. Las tres primeras salen de la matriz de Gram del \emph{batch} de medición. El contraste conserva una métrica por cantidad. La escala de ruido representa a $\alpha$. La m-coherencia y la NGV se miden y se registran igual, pero no entran. La disparidad se queda porque es la única que conserva la escala del gradiente. GSNR y GWA se quedan porque leen cantidades propias. La Sección~\ref{sec:protocolo-objetivos} fija esa poda.
...
\paragraph{\emph{Epochs} hasta el umbral (indicador primario).} El número de \emph{epochs} que tarda
el \emph{accuracy} de validación en alcanzar un nivel objetivo $\tau$ propio de cada conjunto de datos y arquitectura,
...
Si el \emph{accuracy} no cruza el umbral dentro del presupuesto, el valor queda \emph{censurado} y se anota como ausente, nunca como el valor del presupuesto. Los extremos de la rejilla de \emph{learning rates} no alcanzan el umbral por diseño. Descartar un entrenamiento lento pero vivo sesgaría la muestra, y por eso se conserva como censurado; distinto es un entrenamiento que nunca aprendió, que la Sección~\ref{sec:protocolo-poblacion} deja fuera de la población con ese criterio. Un $t^\star$ menor indica un entrenamiento más rápido.

\paragraph{AUC del \emph{loss} de validación.} El área bajo la curva de \emph{loss} de validación
dentro del presupuesto, aproximada por la regla trapezoidal sobre el eje de \emph{epochs}. Resume toda la trayectoria en un escalar. Cuanto menor es el área, más rápido y más bajo desciende el \emph{loss}, siempre que la curva descienda. En las condiciones de entrenamiento de este estudio, sin \emph{weight decay} ni \emph{data augmentation}, el \emph{loss} de validación toca fondo pronto y vuelve a subir. El área recoge entonces sobre todo esa subida. En las celdas de la red \emph{fully connected} y de la ResNet-18 fuera de MNIST el mínimo llega entre la \emph{epoch} 4 y la 8 de mediana. Entre el 84 y el 94\,\% del área queda después de él. Ahí el área ordena los entrenamientos por cuánto sobreajustan más que por lo rápido que aprenden, y la Sección~\ref{sec:variables} la lee así, como apoyo.
...
\paragraph{\emph{Gap} de generalización.} La diferencia final entre el rendimiento en test y en
