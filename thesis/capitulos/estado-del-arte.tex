...
Esa misma ventaja convierte a TSE en el rival a batir. Si la curva de loss temprana ya predice el resultado a coste nulo, cualquier métrica que exija instrumentar el gradiente tiene que demostrar que aporta algo por encima de ella. Este trabajo adopta esa exigencia como criterio central, y por eso TSE reaparece como predictor de referencia en la Sección~\ref{sec:sota-baselines}.
...
La revisión anterior deja ver un patrón. Cada métrica se ha propuesto y validado por separado, con un objetivo propio (explicar la generalización, elegir el tamaño de batch o decidir cuándo parar), sobre un conjunto de datos y una arquitectura concretos, y rara vez frente al predictor de referencia barato. La Tabla~\ref{tab:sota-metricas} las reúne en los mismos ejes y está pensada como referencia a la que volver desde el resto de la memoria: de cada métrica recoge qué mide, cómo se interpreta su valor y cuánto cuesta obtenerlo.

La columna de lectura indica si el trabajo de origen asocia un valor alto o un valor bajo con un entrenamiento más eficiente o una mejor generalización, y precisa a continuación qué significa ese valor. La escala de ruido y la varianza normalizada son la excepción: no se propusieron como medidas de calidad, sino para predecir el tamaño de batch crítico y para describir la varianza, de modo que su lectura señala solo el sentido de menos ruido y no una valoración de mejor o peor. El coste se expresa en relación con el de un paso de entrenamiento y distingue tres tramos: bajo cuando la sobrecarga queda por debajo del cinco por ciento, medio cuando exige un barrido de gradientes por ejemplo sobre el conjunto de medición, y alto cuando además hay que recorrer todos los pares de ese barrido.

Quedan fuera dos ejes que no admiten lectura en paralelo. Los bancos de prueba apenas se solapan: van de una única ResNet-18 sobre ImageNet en la m-coherencia a las 220 configuraciones sobre cuatro arquitecturas de la gradient disparity. Y las correlaciones publicadas miden cosas distintas, porque solo la de GWA relaciona la métrica con el accuracy de test, mientras que la de gradient disparity compara dos variantes de la propia métrica, la de GSNR contrasta los dos lados de la ecuación teórica que sus autores derivan y el resto respalda su propuesta con figuras y no con un coeficiente. Alinear esas cifras en una columna invitaría justo a la comparación que no procede. Esa heterogeneidad, más que la magnitud de ningún coeficiente concreto, es lo que justifica someter a todas las métricas a una evaluación común.
...
\toprule
\textbf{Métrica} & \textbf{Qué mide} & \textbf{Cómo se lee} & \textbf{Coste} \\
\midrule
\multicolumn{4}{l}{\itshape Familia de alineación o coherencia direccional} \\
\addlinespace[2pt]
m-coherencia \cite{chatterjee2020making} & Ejemplos que se benefician del paso obtenido de uno solo & Un valor alto es favorable: indica cuántos ejemplos progresan a la vez con un mismo paso. Cerca de 1 cada ejemplo avanza por separado, lo que apunta a memorización & Medio \\
\addlinespace[2pt]
Stiffness \cite{fort2019stiffness} & Coseno entre gradientes de pares, desglosado por clase & Un valor alto es favorable: los ejemplos de una misma clase se benefician mutuamente de cada paso. Su caída hacia cero anuncia el sobreajuste & Medio \\
\addlinespace[2pt]
Gradient confusion \cite{sankararaman2020impact} & Coseno más negativo entre pares, el peor caso & Un valor bajo es favorable: un valor alto revela pares de ejemplos con gradientes opuestos, que reducen el avance neto de SGD & Alto \\
\addlinespace[2pt]
Gradient disparity \cite{forouzesh2021disparity} & Distancia entre los gradientes de dos minibatches & Un valor bajo es favorable: dos lotes cualesquiera producen gradientes casi idénticos. Su aumento es la señal de parada temprana & Bajo \\
\addlinespace[2pt]
GWA \cite{hoelzl2025gradient} & Coseno entre el gradiente y los pesos del clasificador & Un valor alto es favorable: el gradiente se alinea con la dirección ya fijada por los pesos. Su máximo durante el entrenamiento anticipa el accuracy final & Bajo \\
\addlinespace[3pt]
\multicolumn{4}{l}{\itshape Familia de variabilidad estocástica} \\
\addlinespace[2pt]
Varianza normalizada \cite{faghri2020study} & Varianza del gradiente relativa a su magnitud & Un valor bajo corresponde a menos ruido frente a la señal, si bien en la práctica la métrica crece durante el entrenamiento en lugar de decrecer & Bajo \\
\addlinespace[2pt]
Escala de ruido (GNS) \cite{mccandlish2018empirical} & Traza de la covarianza sobre la norma del gradiente al cuadrado & Un valor bajo corresponde a menos ruido frente a la señal. Su magnitud es el tamaño de lote a partir del cual ampliarlo deja de acelerar el entrenamiento & Medio \\
\addlinespace[2pt]
GSNR \cite{liu2020understanding} & Razón señal-ruido del gradiente por parámetro & Un valor alto es favorable: la media del gradiente domina a su varianza, lo que se asocia con un gap de generalización menor & Medio \\
\addlinespace[3pt]
\multicolumn{4}{l}{\itshape Predictor de referencia, sin instrumentar el gradiente} \\
\addlinespace[2pt]
TSE \cite{ru2021speedy} & Suma del loss de entrenamiento de las primeras épocas & Un valor bajo es favorable: la curva de loss desciende antes, lo que ya ordena las configuraciones sin coste añadido & Nulo \\
\bottomrule
\end{tabular}
\end{table}

La aportación no es, por tanto, una métrica nueva, sino una evaluación comparativa y rigurosa de las existentes. El resultado que se busca no es coronar a la que mejor predice, sino a la que mejor predice por unidad de coste, lo que se expresa como un frente de Pareto entre potencia predictiva y coste de cómputo. Dentro de los límites que la Sección~\ref{sec:alcance} ya declaró, el Capítulo~\ref{ch:metodologia} traduce este posicionamiento en un diseño experimental con hipótesis falsables.
