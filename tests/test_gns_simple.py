...
def test_parallel_grads_zero_noise():
    ...
    assert out["noise_scale/noise"] == pytest.approx(0.0, abs=1e-6)
...
def test_orthonormal_eye_known_value():
    ...
    assert out["noise_scale/noise"] == pytest.approx(0.5)
...
def test_two_row_crafted_value():
    ...
    out = _gns_core(G)
    assert out["noise_scale/noise"] == pytest.approx(1.25)
    assert out["noise_scale/simple"] == pytest.approx(1.0)
...
def test_three_row_crafted_value():
    ...
    out = _gns_core(G)
    assert out["noise_scale/noise"] == pytest.approx(8.0 / 3.0)
...
def test_scale_invariance_simple():
    ...
    assert scaled["noise_scale/noise"] == pytest.approx(c * c * base["noise_scale/noise"])
...
def test_noise_is_biased_low_by_M_minus_1_over_M():
    ...
    out = _gns_core(G)
    assert out["noise_scale/noise"] == pytest.approx((m - 1) / m * unbiased_tr)
...
def test_noise_matches_appendix_a1_estimator_up_to_bessel():
    ...
    # noise_scale/noise must equal (M-1)/M * S built directly from A.2.
    ...
    out = _gns_core(G)
    assert out["noise_scale/noise"] == pytest.approx(float((m - 1) / m * s_unbiased))


def test_single_batch_noise_nonnegative():
    # Unlike the two-batch A.1 estimator, the per-sample noise is a sum of
    # per-coordinate (biased) variances, so it can never go negative even when
    # the mean is ~0 and noise dominates.
    g = torch.Generator().manual_seed(3)
    for _ in range(50):
        G = torch.randn(torch.randint(2, 8, (1,), generator=g).item(),
                        torch.randint(1, 6, (1,), generator=g).item(), generator=g)
        assert _gns_core(G)["noise_scale/noise"] >= 0.0


def test_compute_smoke_finite():
    out = METRIC.compute(tiny_mlp().eval(), *synthetic_probe(), nn.CrossEntropyLoss())
    assert set(out) == {"noise_scale/noise", "noise_scale/simple"}
    for v in out.values():
